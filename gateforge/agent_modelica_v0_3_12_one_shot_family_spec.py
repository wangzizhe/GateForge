...
SCHEMA_VERSION = "agent_modelica_v0_3_12_one_shot_family_spec"
DEFAULT_OUT_DIR = "artifacts/agent_modelica_v0_3_12_one_shot_family_spec"
...
def check_family_gate(candidate: dict) -> tuple[bool, str]:
    family_id = _norm(candidate.get("v0_3_12_family_id") or candidate.get("family_id"))
...
def check_source_bucket_gate(candidate: dict) -> tuple[bool, str]:
    ...
    if bucket == _norm(SOURCE_BUCKET):
        return True, f"source_bucket_ok:{bucket}"
    return False, f"source_bucket_not_supported:{bucket}"


def check_branch_design_gate(candidate: dict) -> tuple[bool, str]:
    ...
    branches = candidate.get("candidate_branches") or candidate.get("candidate_next_branches")
    branches = [row for row in branches if isinstance(row, dict)] if isinstance(branches, list) else []
    ...
    if current_branch != selected_branch:
        return False, "selected_branch_must_equal_current_branch"
    if not preferred_branch or preferred_branch == current_branch:
        return False, "preferred_branch_missing_or_same_as_current"
    if len(branches) < 2:
        return False, "candidate_branches_missing_or_too_small"
    seen = {_norm(row.get("branch_id")) for row in branches}
    if current_branch not in seen or preferred_branch not in seen:
        return False, "current_or_preferred_branch_not_in_candidates"
    return True, "branch_design_ok"
...
def check_measurement_protocol_gate(candidate: dict) -> tuple[bool, str]:
    ...
    if _norm(protocol.get("protocol_version")) != _norm(BASELINE_PROTOCOL_VERSION):
...
def run_candidate_ready_gates(candidate: dict) -> dict:
    ...
    for gate_fn, gate_name in [
        ...
        (check_branch_design_gate, "branch_design_gate"),
...
def build_lane_summary_from_taskset(*, candidate_taskset_path: str = DEFAULT_TASKSET, out_dir: str = DEFAULT_OUT_DIR) -> dict:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate candidate-ready status for the v0.3.12 one-shot family.")
