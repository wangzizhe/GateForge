...
SCHEMA_VERSION = "agent_modelica_wave2_1_curated_retrieval_v1"
DEFAULT_DYNAMIC_ACTIONS = {
    "solver_sensitive_simulate_failure": [
        "restore source solver-facing parameters before widening planner edits",
        "inspect stiff gains and event frequency before retrying simulate",
    ],
    "event_logic_error": [
        "restore event thresholds and reset conditions to source semantics",
        "avoid broad equation rewrites before validating when/reinit behavior",
    ],
    "semantic_drift_after_compile_pass": [
        "restore sign-sensitive dynamic equations to source-local behavior",
        "validate post-compile semantics before widening repair scope",
    ],
}
...
def build_wave2_1_curated_retrieval_rows(manifest_path: str, failure_types: list[str]) -> dict:
    payload = load_wave2_1_harder_dynamics_manifest(manifest_path)
    libraries, reasons = validate_wave2_1_harder_dynamics_manifest(payload)
                ...
                rows.append(
                    {
                        ...
                        "used_strategy": f"wave2_1_curated_{library_id}_{failure_type}",
                        "action_trace": DEFAULT_DYNAMIC_ACTIONS.get(failure_type) or [],
                        "status": "PASS",
                        ...
                        "reason": "wave2_1_curated_support",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build wave2.1 harder-dynamics curated retrieval rows")
    ...
    parser.add_argument("--history-out", default="artifacts/agent_modelica_wave2_1_curated_retrieval_v1/history.json")
    parser.add_argument("--out", default="artifacts/agent_modelica_wave2_1_curated_retrieval_v1/summary.json")
    args = parser.parse_args()
    failure_types = [item.strip().lower() for item in str(args.failure_types or "").split(",") if item.strip()]
    payload = build_wave2_1_curated_retrieval_rows(str(args.manifest), failure_types)
