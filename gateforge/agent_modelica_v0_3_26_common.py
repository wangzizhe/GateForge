...
from .agent_modelica_v0_3_25_common import (
    DUAL_RECHECK_SPECS as V0325_DUAL_RECHECK_SPECS,
    SINGLE_MISMATCH_SPECS as V0325_SINGLE_MISMATCH_SPECS,
    build_v0325_source_specs,
)


SCHEMA_PREFIX = "agent_modelica_v0_3_26"
REPO_ROOT = Path(__file__).resolve().parents[1]

DEFAULT_MANIFEST_OUT_DIR = REPO_ROOT / "artifacts" / "agent_modelica_v0_3_26_coverage_manifest_current"
DEFAULT_SURFACE_INDEX_OUT_DIR = REPO_ROOT / "artifacts" / "agent_modelica_v0_3_26_surface_index_current"
DEFAULT_SURFACE_AUDIT_OUT_DIR = REPO_ROOT / "artifacts" / "agent_modelica_v0_3_26_surface_export_audit_current"
DEFAULT_TASKSET_OUT_DIR = REPO_ROOT / "artifacts" / "agent_modelica_v0_3_26_taskset_current"
DEFAULT_PATCH_CONTRACT_OUT_DIR = REPO_ROOT / "artifacts" / "agent_modelica_v0_3_26_patch_contract_current"
DEFAULT_FIRST_FIX_RESULTS_DIR = REPO_ROOT / "artifacts" / "agent_modelica_v0_3_26_first_fix_results_current"
DEFAULT_FIRST_FIX_OUT_DIR = REPO_ROOT / "artifacts" / "agent_modelica_v0_3_26_first_fix_evidence_current"
DEFAULT_DUAL_RECHECK_RESULTS_DIR = REPO_ROOT / "artifacts" / "agent_modelica_v0_3_26_dual_recheck_results_current"
DEFAULT_DUAL_RECHECK_OUT_DIR = REPO_ROOT / "artifacts" / "agent_modelica_v0_3_26_dual_recheck_current"
DEFAULT_CLOSEOUT_OUT_DIR = REPO_ROOT / "artifacts" / "agent_modelica_v0_3_26_closeout_current"


def _clone_spec_with_v0326_id(spec: dict) -> dict:
    updated = dict(spec)
    updated["task_id"] = norm(spec.get("task_id")).replace("v0325_", "v0326_", 1)
    return updated


SINGLE_MISMATCH_SPECS = [_clone_spec_with_v0326_id(spec) for spec in V0325_SINGLE_MISMATCH_SPECS]
DUAL_RECHECK_SPECS = [_clone_spec_with_v0326_id(spec) for spec in V0325_DUAL_RECHECK_SPECS]
...
def _symbol_occurrence_count(source_model_text: str, symbol: str) -> int:
    if not norm(symbol):
        return 0
    return len(re.findall(re.escape(norm(symbol)), norm(source_model_text)))


def build_surface_index_payload(*, use_fixture_only: bool = False) -> dict:
    records: dict[str, dict] = {}
    export_failures: list[dict] = []
    source_specs = build_v0326_source_specs()
