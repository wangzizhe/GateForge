...
from .agent_modelica_v0_3_26_coverage_manifest import build_v0326_coverage_manifest
from .agent_modelica_v0_3_26_dual_recheck import build_v0326_dual_recheck
from .agent_modelica_v0_3_26_first_fix_evidence import build_v0326_first_fix_evidence
from .agent_modelica_v0_3_26_patch_contract import build_v0326_patch_contract
from .agent_modelica_v0_3_26_surface_export_audit import build_v0326_surface_export_audit


SCHEMA_VERSION = f"{SCHEMA_PREFIX}_closeout"


def build_v0326_closeout(
    *,
    ...
    if not Path(manifest_path).exists():
        build_v0326_coverage_manifest(out_dir=str(Path(manifest_path).parent))
    if not Path(surface_audit_path).exists():
        build_v0326_surface_export_audit(out_dir=str(Path(surface_audit_path).parent))
    if not Path(patch_contract_path).exists():
        build_v0326_patch_contract(out_dir=str(Path(patch_contract_path).parent))
    if not Path(first_fix_path).exists():
        build_v0326_first_fix_evidence(out_dir=str(Path(first_fix_path).parent))
    if not Path(dual_recheck_path).exists():
        build_v0326_dual_recheck(out_dir=str(Path(dual_recheck_path).parent))
    ...
    if boundary_rejected:
        version_decision = "stage2_component_type_local_interface_discovery_boundary_rejected"
    elif export_ready and first_fix_ready and dual_ready:
        version_decision = "stage2_component_type_local_interface_discovery_ready"
    elif export_ready and first_fix_ready:
        version_decision = "stage2_component_type_local_interface_discovery_partially_ready"
    else:
        version_decision = "stage2_component_type_local_interface_discovery_not_ready"
    payload = {
        "schema_version": SCHEMA_VERSION,
        "generated_at_utc": now_utc(),
        "status": "PASS",
        "closeout_status": "STAGE2_COMPONENT_TYPE_LOCAL_INTERFACE_DISCOVERY_CLOSEOUT_READY",
        "coverage_manifest": {
        ...
        "conclusion": {
            ...
            "summary": (
                "Component-type local interface discovery holds without relying on canonical endpoint reuse from the source model: export, candidate recall, top1 ranking, patch execution, and dual residual retention remain stable."
                if version_decision == "stage2_component_type_local_interface_discovery_ready"
                else (
                    "Component-type local interface discovery works at first-fix level, but one downstream layer drops before the lane can be promoted as stable."
                    if version_decision == "stage2_component_type_local_interface_discovery_partially_ready"
                    else (
                        "The family crossed into topology-heavy or compile-failure drift; the next version should retreat to a narrower local interface subtype."
                        if version_decision == "stage2_component_type_local_interface_discovery_boundary_rejected"
                        else "The component-type local interface discovery lane does not yet hold once the source-model-local scaffold is removed."
                    )
                )
            ),
        },
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the v0.3.26 closeout.")
