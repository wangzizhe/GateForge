...
from .agent_modelica_unknown_library_taskset_v1 import (
    ...
    _load_json,
    _mutate_connector_mismatch,
    _mutate_initialization_infeasible,
    _mutate_underconstrained,
    _norm,
    _package_prefixes,
    _ratio,
    _sha256,
...
SCHEMA_VERSION = "agent_modelica_curated_hard_unseen_taskset_v1"
ALLOWED_SEEN_RISK_BANDS = {"less_likely_seen", "hard_unseen"}


def build_curated_hard_unseen_taskset(
    *,
    manifest_path: str,
    out_dir: str,
    ...
    libraries, manifest_reasons = validate_curated_hard_unseen_manifest(payload)
    ...
    for library in libraries:
        for model in library.get("allowed_models") or []:
            if not isinstance(model, dict):
                continue
            band = _norm(model.get("seen_risk_band") or library.get("seen_risk_band")).lower()
            ...
            if model_id in excluded_model_ids or qualified in excluded_qualified:
                excluded_models.append({"library_id": library_id, "model_id": model_id, "qualified_model_name": _norm(model.get("qualified_model_name"))})
                continue
    ...
    for library, model in selected_models:
        ...
        selection_reason = _norm(model.get("selection_reason") or library.get("selection_reason"))
        exposure_notes = _norm(model.get("exposure_notes") or library.get("exposure_notes"))
        ...
        for failure_type in failure_types:
            ...
            task_id = f"hardunseen_{library_id}_{model_id}_{failure_type}"
    ...
    return {
        ...
        "library_count": len({_norm(library.get('library_id')).lower() for library, _model in selected_models}),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build curated hard-unseen frozen taskset from private manifest")
    parser.add_argument("--manifest", required=True)
    parser.add_argument("--out-dir", default="artifacts/agent_modelica_curated_hard_unseen_taskset_v1")
    ...
    parser.add_argument("--seed", default="agent_modelica_curated_hard_unseen_taskset_v1")
    ...
    built = build_curated_hard_unseen_taskset(
    ...
    taskset_unfrozen = {
        ...
        "mode": "curated_hard_unseen_unfrozen",
    ...
    taskset_frozen = {
        "schema_version": SCHEMA_VERSION,
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "mode": "curated_hard_unseen_frozen",
        "tasks": tasks,
        "sources": {"manifest": built.get("manifest_path")},
        "split_freeze": {
    ...
    manifest_payload = {
        ...
        "builder_provenance": {"builder_source_path": str(builder_source_path), "builder_source_sha": _sha256(builder_source_path)},
        "files": {
