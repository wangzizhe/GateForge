...
from .agent_modelica_v0_3_13_runtime_curriculum_taskset import (
    COURSE_STAGE,
    DEFAULT_OUT_DIR as DEFAULT_TASKSET_OUT_DIR,
    FAMILY_ID,
    REQUIRED_HIDDEN_BASE_OPERATOR,
    REQUIRED_SIGNAL_CLUSTER,
)


SCHEMA_VERSION = "agent_modelica_v0_3_13_runtime_curriculum_family_spec"
DEFAULT_TASKSET = f"{DEFAULT_TASKSET_OUT_DIR}/taskset.json"
DEFAULT_OUT_DIR = "artifacts/agent_modelica_v0_3_13_runtime_curriculum_family_spec"
...
def check_runtime_contract_gate(candidate: dict) -> tuple[bool, str]:
    if _norm(candidate.get("hidden_base_operator")) != _norm(REQUIRED_HIDDEN_BASE_OPERATOR):
        return False, f"hidden_base_operator_mismatch:{_norm(candidate.get('hidden_base_operator'))}"
    preview_contract = candidate.get("preview_contract")
    if not isinstance(preview_contract, dict):
        return False, "preview_contract_missing"
    if not bool(preview_contract.get("preview_admission")):
        return False, "preview_admission_required"
    if _norm(preview_contract.get("residual_signal_cluster_id")) != _norm(REQUIRED_SIGNAL_CLUSTER):
        return False, f"signal_cluster_mismatch:{_norm(preview_contract.get('residual_signal_cluster_id'))}"
    if _norm(preview_contract.get("post_rule_residual_stage")) != "stage_5_runtime_numerical_instability":
        return False, f"runtime_stage_required:{_norm(preview_contract.get('post_rule_residual_stage'))}"
    return True, "runtime_contract_ok"


def check_parameter_surface_gate(candidate: dict) -> tuple[bool, str]:
    names = candidate.get("runtime_recovery_parameter_names")
    names = [str(x).strip() for x in names if str(x).strip()] if isinstance(names, list) else []
    if len(names) < 2:
        return False, f"multi_parameter_runtime_names_required:{len(names)}"
    return True, "parameter_surface_ok"


def run_runtime_gates(candidate: dict) -> dict:
    gates = []
    reasons = []
    for gate_fn, gate_name in [
        (check_family_gate, "family_gate"),
        (check_course_stage_gate, "course_stage_gate"),
        (check_runtime_contract_gate, "runtime_contract_gate"),
        (check_parameter_surface_gate, "parameter_surface_gate"),
    ]:
...
def build_family_summary(candidates: list[dict]) -> dict:
    results = [run_runtime_gates(row) for row in candidates]
    ...
    return {
        ...
        "targets": {
            "min_curriculum_ready_cases": MIN_CURRICULUM_READY_CASES,
...
def build_family_summary_from_taskset(*, candidate_taskset_path: str = DEFAULT_TASKSET, out_dir: str = DEFAULT_OUT_DIR) -> dict:
    ...
    _write_text(
        ...
        "\n".join(
            [
                "# v0.3.13 Runtime Curriculum Family Spec",
                "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate the v0.3.13 runtime curriculum family spec.")
