...
def build_v0325_first_fix_evidence(
    *,
    ...
    for task in tasks:
        initial = run_synthetic_task_live(
            ...
            evaluation_label="v0325_single_first_failure",
            max_rounds=1,
        ...
        post = rerun_once(
            ...
            result_dir=results_dir,
            evaluation_label="v0325_single_after_first_fix",
            timeout_sec=timeout_sec,
        ...
        if bool(row.get("patch_applied")) and bool(row.get("signature_advance")):
            admitted = dict(task)
            admitted["v0_3_25_first_fix_row"] = row
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the v0.3.25 widened single-mismatch interface-discovery evidence flow.")
    ...
    payload = build_v0325_first_fix_evidence(
