...
def build_v160_handoff_integrity(
    *,
    v152_closeout_path: str = str(DEFAULT_V152_CLOSEOUT_PATH),
    out_dir: str = str(DEFAULT_HANDOFF_INTEGRITY_OUT_DIR),
) -> dict:
    conclusion = load_json(v152_closeout_path).get("conclusion", {})
    checks = {
        "version_decision_ok": conclusion.get("version_decision") == EXPECTED_V152_VERSION_DECISION,
        "phase_stop_condition_ok": conclusion.get("phase_stop_condition_status") == EXPECTED_V152_PHASE_STOP_CONDITION_STATUS,
        "explicit_caveat_present_ok": bool(conclusion.get("explicit_caveat_present")),
        "explicit_caveat_label_ok": conclusion.get("explicit_caveat_label") == EXPECTED_V152_CAVEAT,
        "next_primary_phase_question_ok": conclusion.get("next_primary_phase_question") == EXPECTED_V152_NEXT_PRIMARY_QUESTION,
        "no_reopen_rule_ok": bool(conclusion.get("do_not_continue_v0_15_same_even_broader_refinement_by_default")),
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the v0.16.0 handoff-integrity artifact.")
