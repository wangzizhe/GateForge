...
from .agent_modelica_v0_3_27_common import (
    DUAL_RECHECK_SPECS as BASE_V0327_DUAL_RECHECK_SPECS,
    build_v0327_source_specs as build_v0327_base_source_specs,
)


SCHEMA_PREFIX = "agent_modelica_v0_3_28"
REPO_ROOT = Path(__file__).resolve().parents[1]

DEFAULT_MANIFEST_OUT_DIR = REPO_ROOT / "artifacts" / "agent_modelica_v0_3_28_coverage_manifest_current"
DEFAULT_SURFACE_INDEX_OUT_DIR = REPO_ROOT / "artifacts" / "agent_modelica_v0_3_28_surface_index_current"
DEFAULT_SURFACE_AUDIT_OUT_DIR = REPO_ROOT / "artifacts" / "agent_modelica_v0_3_28_surface_export_audit_current"
DEFAULT_TASKSET_OUT_DIR = REPO_ROOT / "artifacts" / "agent_modelica_v0_3_28_taskset_current"
DEFAULT_TASKSET_PRECHECK_RESULTS_DIR = REPO_ROOT / "artifacts" / "agent_modelica_v0_3_28_taskset_precheck_results_current"
DEFAULT_PATCH_CONTRACT_OUT_DIR = REPO_ROOT / "artifacts" / "agent_modelica_v0_3_28_patch_contract_current"
DEFAULT_FIRST_FIX_RESULTS_DIR = REPO_ROOT / "artifacts" / "agent_modelica_v0_3_28_first_fix_results_current"
DEFAULT_FIRST_FIX_OUT_DIR = REPO_ROOT / "artifacts" / "agent_modelica_v0_3_28_first_fix_evidence_current"
DEFAULT_DUAL_RECHECK_RESULTS_DIR = REPO_ROOT / "artifacts" / "agent_modelica_v0_3_28_dual_recheck_results_current"
DEFAULT_DUAL_RECHECK_OUT_DIR = REPO_ROOT / "artifacts" / "agent_modelica_v0_3_28_dual_recheck_current"
DEFAULT_CLOSEOUT_OUT_DIR = REPO_ROOT / "artifacts" / "agent_modelica_v0_3_28_closeout_current"
...
def _source_row(source_id: str) -> dict:
    for row in build_v0328_source_specs():
...
ADDITIONAL_SOURCE_SPECS = [
    {
        "source_id": "simple_signal_actuation_chain",
        "complexity_tier": "simple",
        "model_name": "NeighborWidenSimpleSignalActuationChain",
        "source_model_text": "\n".join(
            [
                "model NeighborWidenSimpleSignalActuationChain",
                "  Modelica.Blocks.Sources.Step reference(height = 1.0, startTime = 0.1, offset = 0.0);",
                "  Modelica.Blocks.Math.Gain prefilter(k = 1.0);",
                "  Modelica.Blocks.Math.Gain actuator1(k = 2.0);",
                "  Modelica.Blocks.Math.Gain actuator2(k = 3.0);",
                "equation",
                "  connect(reference.y, prefilter.u);",
                "  connect(prefilter.y, actuator1.u);",
                "  connect(actuator1.y, actuator2.u);",
                "end NeighborWidenSimpleSignalActuationChain;",
            ]
        ),
    },
    {
        "source_id": "simple_thermal_resistor_chain",
        "complexity_tier": "simple",
        "model_name": "NeighborWidenSimpleThermalResistorChain",
        "source_model_text": "\n".join(
            [
                "model NeighborWidenSimpleThermalResistorChain",
                "  Modelica.Thermal.HeatTransfer.Sources.FixedTemperature ambientHot(T = 320.15);",
                "  Modelica.Thermal.HeatTransfer.Components.ThermalResistor wall1(R = 0.1);",
                "  Modelica.Thermal.HeatTransfer.Components.ThermalResistor wall2(R = 0.2);",
                "  Modelica.Thermal.HeatTransfer.Sources.FixedTemperature ambientCold(T = 293.15);",
                "equation",
                "  connect(ambientHot.port, wall1.port_a);",
                "  connect(wall1.port_b, wall2.port_a);",
                "  connect(wall2.port_b, ambientCold.port);",
                "end NeighborWidenSimpleThermalResistorChain;",
            ]
        ),
    },
    {
        "source_id": "simple_electrical_series_chain",
        "complexity_tier": "simple",
        "model_name": "NeighborWidenSimpleElectricalSeriesChain",
        "source_model_text": "\n".join(
            [
                "model NeighborWidenSimpleElectricalSeriesChain",
                "  Modelica.Electrical.Analog.Sources.SineVoltage source(V = 10.0, f = 50.0, phase = 0.0);",
                "  Modelica.Electrical.Analog.Basic.Resistor resistor1(R = 10.0);",
                "  Modelica.Electrical.Analog.Basic.Resistor resistor2(R = 5.0);",
                ...
                "end NeighborWidenSimpleElectricalSeriesChain;",
            ]
        ),
    },
]
...
def _clone_base_dual_spec(spec: dict) -> dict:
    updated = copy.deepcopy(spec)
    updated["task_id"] = norm(spec.get("task_id")).replace("v0327_", "v0328_", 1)
    return updated


ADDITIONAL_DUAL_RECHECK_SPECS = [
    {
        "task_id": "v0328_dual_simple_reference_prefilter_neighbor",
        "source_id": "simple_signal_actuation_chain",
        "complexity_tier": "simple",
        "component_family": "local_signal_port_alignment",
        "placement_kind": "neighbor_component_dual_mismatch",
        "repair_steps": [
            {"patch_type": "replace_local_port_symbol", "wrong_symbol": "reference.outSig", "correct_symbol": "reference.y", "candidate_key": "reference.outSig"},
            {"patch_type": "replace_local_port_symbol", "wrong_symbol": "prefilter.inputSignal", "correct_symbol": "prefilter.u", "candidate_key": "prefilter.inputSignal"},
        ],
        "single_step_injection_replacements": [
            [("connect(reference.y, prefilter.u);", "connect(reference.outSig, prefilter.u);")],
            [("connect(reference.y, prefilter.u);", "connect(reference.y, prefilter.inputSignal);")],
        ],
        "injection_replacements": [("connect(reference.y, prefilter.u);", "connect(reference.outSig, prefilter.inputSignal);")],
    },
    {
        "task_id": "v0328_dual_simple_prefilter_actuator1_neighbor",
        "source_id": "simple_signal_actuation_chain",
        "complexity_tier": "simple",
        "component_family": "local_signal_port_alignment",
        "placement_kind": "neighbor_component_dual_mismatch",
        "repair_steps": [
            {"patch_type": "replace_local_port_symbol", "wrong_symbol": "prefilter.outSig", "correct_symbol": "prefilter.y", "candidate_key": "prefilter.outSig"},
            {"patch_type": "replace_local_port_symbol", "wrong_symbol": "actuator1.inputSignal", "correct_symbol": "actuator1.u", "candidate_key": "actuator1.inputSignal"},
        ],
        "single_step_injection_replacements": [
            [("connect(prefilter.y, actuator1.u);", "connect(prefilter.outSig, actuator1.u);")],
            [("connect(prefilter.y, actuator1.u);", "connect(prefilter.y, actuator1.inputSignal);")],
        ],
        "injection_replacements": [("connect(prefilter.y, actuator1.u);", "connect(prefilter.outSig, actuator1.inputSignal);")],
    },
    {
        "task_id": "v0328_dual_simple_actuator1_actuator2_neighbor",
        "source_id": "simple_signal_actuation_chain",
        "complexity_tier": "simple",
        "component_family": "local_signal_port_alignment",
        "placement_kind": "neighbor_component_dual_mismatch",
        "repair_steps": [
            {"patch_type": "replace_local_port_symbol", "wrong_symbol": "actuator1.out", "correct_symbol": "actuator1.y", "candidate_key": "actuator1.out"},
            {"patch_type": "replace_local_port_symbol", "wrong_symbol": "actuator2.inputSignal", "correct_symbol": "actuator2.u", "candidate_key": "actuator2.inputSignal"},
        ],
        "single_step_injection_replacements": [
            [("connect(actuator1.y, actuator2.u);", "connect(actuator1.out, actuator2.u);")],
            [("connect(actuator1.y, actuator2.u);", "connect(actuator1.y, actuator2.inputSignal);")],
        ],
        "injection_replacements": [("connect(actuator1.y, actuator2.u);", "connect(actuator1.out, actuator2.inputSignal);")],
    },
    {
        "task_id": "v0328_dual_simple_ambienthot_wall1_neighbor",
        "source_id": "simple_thermal_resistor_chain",
        "complexity_tier": "simple",
        "component_family": "local_connector_side_alignment",
        "placement_kind": "neighbor_component_dual_mismatch",
        "repair_steps": [
            {"patch_type": "replace_local_port_symbol", "wrong_symbol": "ambientHot.portHot", "correct_symbol": "ambientHot.port", "candidate_key": "ambientHot.portHot"},
            {"patch_type": "replace_connect_endpoint", "wrong_symbol": "wall1.portLeft", "correct_symbol": "wall1.port_a", "candidate_key": "wall1.portLeft"},
        ],
        "single_step_injection_replacements": [
            [("connect(ambientHot.port, wall1.port_a);", "connect(ambientHot.portHot, wall1.port_a);")],
            [("connect(ambientHot.port, wall1.port_a);", "connect(ambientHot.port, wall1.portLeft);")],
        ],
        "injection_replacements": [("connect(ambientHot.port, wall1.port_a);", "connect(ambientHot.portHot, wall1.portLeft);")],
    },
    {
        "task_id": "v0328_dual_simple_wall1_wall2_neighbor",
        "source_id": "simple_thermal_resistor_chain",
        "complexity_tier": "simple",
        "component_family": "local_connector_side_alignment",
        "placement_kind": "neighbor_component_dual_mismatch",
        "repair_steps": [
            {"patch_type": "replace_connect_endpoint", "wrong_symbol": "wall1.portRight", "correct_symbol": "wall1.port_b", "candidate_key": "wall1.portRight"},
            {"patch_type": "replace_connect_endpoint", "wrong_symbol": "wall2.portLeft", "correct_symbol": "wall2.port_a", "candidate_key": "wall2.portLeft"},
        ],
        "single_step_injection_replacements": [
            [("connect(wall1.port_b, wall2.port_a);", "connect(wall1.portRight, wall2.port_a);")],
            [("connect(wall1.port_b, wall2.port_a);", "connect(wall1.port_b, wall2.portLeft);")],
        ],
        "injection_replacements": [("connect(wall1.port_b, wall2.port_a);", "connect(wall1.portRight, wall2.portLeft);")],
    },
    {
        "task_id": "v0328_dual_simple_wall2_ambientcold_neighbor",
        "source_id": "simple_thermal_resistor_chain",
        "complexity_tier": "simple",
        "component_family": "local_connector_side_alignment",
        "placement_kind": "neighbor_component_dual_mismatch",
        "repair_steps": [
            {"patch_type": "replace_connect_endpoint", "wrong_symbol": "wall2.portRight", "correct_symbol": "wall2.port_b", "candidate_key": "wall2.portRight"},
            {"patch_type": "replace_local_port_symbol", "wrong_symbol": "ambientCold.portHot", "correct_symbol": "ambientCold.port", "candidate_key": "ambientCold.portHot"},
        ],
        "single_step_injection_replacements": [
            [("connect(wall2.port_b, ambientCold.port);", "connect(wall2.portRight, ambientCold.port);")],
            [("connect(wall2.port_b, ambientCold.port);", "connect(wall2.port_b, ambientCold.portHot);")],
        ],
        "injection_replacements": [("connect(wall2.port_b, ambientCold.port);", "connect(wall2.portRight, ambientCold.portHot);")],
    },
    {
        "task_id": "v0328_dual_simple_source_resistor1_neighbor",
        "source_id": "simple_electrical_series_chain",
        "complexity_tier": "simple",
        "component_family": "local_connector_side_alignment",
        "placement_kind": "neighbor_component_dual_mismatch",
        "repair_steps": [
            {"patch_type": "replace_connect_endpoint", "wrong_symbol": "source.posPin", "correct_symbol": "source.p", "candidate_key": "source.posPin"},
            {"patch_type": "replace_connect_endpoint", "wrong_symbol": "resistor1.posPin", "correct_symbol": "resistor1.p", "candidate_key": "resistor1.posPin"},
        ],
        "single_step_injection_replacements": [
            [("connect(source.p, resistor1.p);", "connect(source.posPin, resistor1.p);")],
            [("connect(source.p, resistor1.p);", "connect(source.p, resistor1.posPin);")],
        ],
        "injection_replacements": [("connect(source.p, resistor1.p);", "connect(source.posPin, resistor1.posPin);")],
    },
    {
        "task_id": "v0328_dual_simple_resistor1_resistor2_neighbor",
        "source_id": "simple_electrical_series_chain",
        "complexity_tier": "simple",
        "component_family": "local_connector_side_alignment",
        "placement_kind": "neighbor_component_dual_mismatch",
        "repair_steps": [
            {"patch_type": "replace_connect_endpoint", "wrong_symbol": "resistor1.negPin", "correct_symbol": "resistor1.n", "candidate_key": "resistor1.negPin"},
            {"patch_type": "replace_connect_endpoint", "wrong_symbol": "resistor2.posPin", "correct_symbol": "resistor2.p", "candidate_key": "resistor2.posPin"},
        ],
        "single_step_injection_replacements": [
            [("connect(resistor1.n, resistor2.p);", "connect(resistor1.negPin, resistor2.p);")],
            [("connect(resistor1.n, resistor2.p);", "connect(resistor1.n, resistor2.posPin);")],
        ],
        "injection_replacements": [("connect(resistor1.n, resistor2.p);", "connect(resistor1.negPin, resistor2.posPin);")],
    },
    {
        "task_id": "v0328_dual_simple_resistor2_inductor_neighbor",
        "source_id": "simple_electrical_series_chain",
        "complexity_tier": "simple",
        "component_family": "local_connector_side_alignment",
        "placement_kind": "neighbor_component_dual_mismatch",
        "repair_steps": [
            {"patch_type": "replace_connect_endpoint", "wrong_symbol": "resistor2.negPin", "correct_symbol": "resistor2.n", "candidate_key": "resistor2.negPin"},
            {"patch_type": "replace_connect_endpoint", "wrong_symbol": "inductor.posPin", "correct_symbol": "inductor.p", "candidate_key": "inductor.posPin"},
        ],
        "single_step_injection_replacements": [
            [("connect(resistor2.n, inductor.p);", "connect(resistor2.negPin, inductor.p);")],
            [("connect(resistor2.n, inductor.p);", "connect(resistor2.n, inductor.posPin);")],
        ],
        "injection_replacements": [("connect(resistor2.n, inductor.p);", "connect(resistor2.negPin, inductor.posPin);")],
    },
]


def build_v0328_source_specs() -> list[dict]:
    return [dict(row) for row in build_v0327_base_source_specs()] + [dict(row) for row in ADDITIONAL_SOURCE_SPECS]


DUAL_RECHECK_SPECS = [_clone_base_dual_spec(spec) for spec in BASE_V0327_DUAL_RECHECK_SPECS] + [
...
def build_surface_index_payload(*, use_fixture_only: bool = False) -> dict:
    ...
    source_specs = build_v0328_source_specs()
...
def precheck_neighbor_dual_second_residual(task: dict, *, results_dir: str, timeout_sec: int = 600, use_fixture_only: bool = False) -> dict:
    ...
    rerun = rerun_once(
        ...
        evaluation_label="v0328_neighbor_precheck_after_first_fix",
