...
from .agent_modelica_v0_10_2_handoff_integrity import build_v102_handoff_integrity
from .agent_modelica_v0_10_2_pool_delta_and_diversity_report import build_v102_pool_delta_and_diversity_report


def build_v102_closeout(
    *,
    ...
    if handoff.get("handoff_integrity_status") != "PASS":
        payload = {
            ...
            "status": "FAIL",
            "closeout_status": "V0_10_2_SOURCE_EXPANSION_INPUTS_INVALID",
            "conclusion": {
                "version_decision": "v0_10_2_source_expansion_inputs_invalid",
                "real_origin_source_expansion_status": "invalid",
                "v0_10_3_handoff_mode": "rebuild_v0_10_2_source_expansion_inputs_first",
            },
            "handoff_integrity": handoff,
        }
        write_json(out_root / "summary.json", payload)
        write_text(out_root / "summary.md", "# v0.10.2 Closeout\n\n- version_decision: `v0_10_2_source_expansion_inputs_invalid`\n")
        return payload

    if not source_admission_path_obj.exists() or source_admission_path_obj == default_source_admission_path:
        build_v102_candidate_source_admission(out_dir=str(source_admission_path_obj.parent))
    ...
    if not pool_delta_path_obj.exists() or pool_delta_path_obj == default_pool_delta_path:
        build_v102_pool_delta_and_diversity_report(
            v101_closeout_path=v101_closeout_path,
            v101_pool_delta_path=v101_pool_delta_path,
    ...
    if expansion_status == "expansion_ready":
        version_decision = "v0_10_2_real_origin_source_expansion_ready"
        handoff_mode = "freeze_first_real_origin_workflow_substrate"
        status = "PASS"
    elif expansion_status == "expansion_partial":
        version_decision = "v0_10_2_real_origin_source_expansion_partial"
        handoff_mode = "continue_expanding_real_origin_candidate_pool"
        status = "PASS"
    else:
        version_decision = "v0_10_2_source_expansion_inputs_invalid"
        handoff_mode = "rebuild_v0_10_2_source_expansion_inputs_first"
        status = "FAIL"
    ...
    payload = {
        ...
        "conclusion": {
            ...
            "why_this_is_or_is_not_valid": (
                "The real-origin pool keeps growing under the frozen governance pack, but the substrate-freeze floor is not yet fully met."
                if expansion_status == "expansion_partial"
                else "The real-origin pool now reaches the first substrate-freeze readiness floor with preserved source purity and non-worsened diversity."
                if expansion_status == "expansion_ready"
                else "Handoff or source-expansion inputs are invalid and must be rebuilt before real-origin substrate freeze can proceed."
            ),
            "v0_10_3_handoff_mode": handoff_mode,
        },
        "handoff_integrity": handoff,
    ...
    write_text(
        ...
        "\n".join(
            [
                ...
                f"- v0_10_3_handoff_mode: `{handoff_mode}`",
            ]
        ),
    )
    return payload
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the v0.10.2 source expansion closeout.")
