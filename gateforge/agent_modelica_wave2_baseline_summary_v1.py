...
SCHEMA_VERSION = "agent_modelica_wave2_baseline_summary_v1"
...
def _record_buckets(taskset_payload: dict, results_payload: dict) -> tuple[dict[str, dict], dict[str, dict], dict[str, dict]]:
    ...
    for record in [row for row in (results_payload.get("records") or []) if isinstance(row, dict)]:
        ...
        fail_row = failure_breakdown_by_failure_type.setdefault(
            failure_type,
            {"task_count": 0, "diagnostic_parse_fail": 0, "deterministic_repair_missing": 0, "llm_patch_drift": 0, "source_unstable": 0, "infra": 0},
        )
        fail_row["task_count"] += 1
        if bool(record.get("passed")):
            continue
        err = str(record.get("error_message") or "").strip().lower()
        if "timeout" in err or "infra" in err or "docker" in err:
            fail_row["infra"] += 1
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Summarize wave2 baseline results")
    parser.add_argument("--challenge-summary", required=True)
    parser.add_argument("--baseline-summary", required=True)
    parser.add_argument("--baseline-results", required=True)
    parser.add_argument("--out", default="artifacts/agent_modelica_wave2_realism_live_evidence_v1/wave2_baseline_summary.json")
    ...
    summary = {
        ...
        "counts_by_error_family": challenge.get("counts_by_error_family") if isinstance(challenge.get("counts_by_error_family"), dict) else {},
