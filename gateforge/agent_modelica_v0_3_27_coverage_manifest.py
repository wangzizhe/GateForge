...
def build_v0327_coverage_manifest(*, out_dir: str = str(DEFAULT_MANIFEST_OUT_DIR)) -> dict:
    sources = build_v0327_source_specs()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the v0.3.27 neighbor-component local-interface discovery manifest.")
    parser.add_argument("--out-dir", default=str(DEFAULT_MANIFEST_OUT_DIR))
    args = parser.parse_args()
    payload = build_v0327_coverage_manifest(out_dir=str(args.out_dir))
