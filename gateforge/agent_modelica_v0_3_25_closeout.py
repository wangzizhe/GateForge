...
from .agent_modelica_v0_3_25_coverage_manifest import build_v0325_coverage_manifest
from .agent_modelica_v0_3_25_dual_recheck import build_v0325_dual_recheck
from .agent_modelica_v0_3_25_first_fix_evidence import build_v0325_first_fix_evidence
from .agent_modelica_v0_3_25_patch_contract import build_v0325_patch_contract
from .agent_modelica_v0_3_25_surface_export_audit import build_v0325_surface_export_audit


SCHEMA_VERSION = f"{SCHEMA_PREFIX}_closeout"


def build_v0325_closeout(
    *,
    ...
    if not Path(manifest_path).exists():
        build_v0325_coverage_manifest(out_dir=str(Path(manifest_path).parent))
    if not Path(surface_audit_path).exists():
        build_v0325_surface_export_audit(out_dir=str(Path(surface_audit_path).parent))
    if not Path(patch_contract_path).exists():
        build_v0325_patch_contract(out_dir=str(Path(patch_contract_path).parent))
    if not Path(first_fix_path).exists():
        build_v0325_first_fix_evidence(out_dir=str(Path(first_fix_path).parent))
    if not Path(dual_recheck_path).exists():
        build_v0325_dual_recheck(out_dir=str(Path(dual_recheck_path).parent))
    ...
    export_ready = (
        ...
        and int(surface.get("active_single_task_count") or 0) >= 18
        and int(surface.get("active_dual_sidecar_task_count") or 0) >= 12
    )
    boundary_rejected = (
    ...
    if boundary_rejected:
        version_decision = "stage2_local_interface_discovery_coverage_boundary_rejected"
    elif export_ready and first_fix_ready and dual_ready:
        version_decision = "stage2_local_interface_discovery_coverage_ready"
    elif export_ready and first_fix_ready:
        version_decision = "stage2_local_interface_discovery_coverage_partially_ready"
    else:
        version_decision = "stage2_local_interface_discovery_coverage_not_ready"
    payload = {
        "schema_version": SCHEMA_VERSION,
        "generated_at_utc": now_utc(),
        "status": "PASS",
        "closeout_status": "STAGE2_LOCAL_INTERFACE_DISCOVERY_COVERAGE_CLOSEOUT_READY",
        "coverage_manifest": {
        ...
        "dual_recheck": {
            ...
            "same_cluster_second_residual_local_interface_retained_rate_pct": float(dual.get("same_cluster_second_residual_local_interface_retained_rate_pct") or 0.0),
        ...
        "conclusion": {
            ...
            "summary": (
                "The widened local_interface_discovery lane still holds: export, local candidate recall, top1 ranking, patch execution, and dual residual retention remain stable beyond the initial narrow slice."
                if version_decision == "stage2_local_interface_discovery_coverage_ready"
                else (
                    "Widened local interface discovery still works at first-fix level, but one downstream layer drops before the lane can be promoted as stable widened coverage."
                    if version_decision == "stage2_local_interface_discovery_coverage_partially_ready"
                    else (
                        "The widened family crossed into topology-heavy or compile-failure drift; the next version should retreat to a narrower local interface subtype."
                        if version_decision == "stage2_local_interface_discovery_coverage_boundary_rejected"
                        else "The widened local interface discovery slice does not yet hold once the narrow v0.3.24 lane is expanded."
                    )
                )
            ),
        },
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the v0.3.25 closeout.")
