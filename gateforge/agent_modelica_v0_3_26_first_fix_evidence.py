...
def build_v0326_first_fix_evidence(
    *,
    ...
    for task in tasks:
        initial = run_synthetic_task_live(
            ...
            evaluation_label="v0326_single_first_failure",
            max_rounds=1,
        ...
        post = (
            rerun_once(
                ...
                result_dir=results_dir,
                evaluation_label="v0326_single_after_first_fix",
                timeout_sec=timeout_sec,
            )
        ...
        if bool(row.get("patch_applied")) and bool(row.get("signature_advance")):
            admitted = dict(task)
            admitted["v0_3_26_first_fix_row"] = row
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the v0.3.26 component-type local-interface first-fix evidence flow.")
    ...
    payload = build_v0326_first_fix_evidence(
