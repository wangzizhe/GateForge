...
def build_v0326_taskset(
    *,
            ...
            export_failures.append(
                {
                    "task_id": norm(task.get("task_id")),
                    "component_family": norm(task.get("component_family")),
                    "reason": "canonical_endpoint_present_elsewhere_in_source_model",
                }
            )
            continue
        row = dict(task)
    ...
    summary = {
        ...
        "source_count": len(build_v0326_source_specs()),
        "source_tier_counts": _count_by(build_v0326_source_specs(), "complexity_tier"),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the v0.3.26 component-type local-interface discovery taskset.")
