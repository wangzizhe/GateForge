...
def load_wave2_2_coupled_hard_manifest(path: str) -> dict:
    payload = json.loads(Path(path).read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError("wave2.2 coupled-hard manifest must be a JSON object")
    payload["_manifest_path"] = str(Path(path).resolve())
    return payload


def validate_wave2_2_coupled_hard_manifest(payload: dict) -> tuple[list[dict], list[str]]:
    reasons: list[str] = []
    ...
    for idx, library in enumerate(libraries):
        ...
        if checked_models:
            ...
            library_row["allowed_models"] = checked_models
            validated.append(library_row)
    if len(validated) < 2:
