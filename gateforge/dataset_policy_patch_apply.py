...
APPROVAL_PROFILE_DIR = Path("policies/dataset_patch_apply")
...
def _write_markdown(path: str, summary: dict) -> None:
    ...
    p.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# GateForge Dataset Policy Patch Apply",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Apply dataset policy patch proposal with human approval gate")
    parser.add_argument("--proposal", required=True, help="Dataset policy patch proposal JSON path")
    parser.add_argument("--approval", default=None, help="Approval decision JSON path")
    parser.add_argument("--approval-profile", default=None, help="Approval profile under policies/dataset_patch_apply")
    parser.add_argument("--approval-policy-path", default=None, help="Approval policy JSON path override")
    parser.add_argument(
        "--apply",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="When enabled and approval passes, write policy_after to target policy path",
    )
    parser.add_argument("--out", default="artifacts/dataset_policy_patch/apply.json", help="Apply summary JSON path")
