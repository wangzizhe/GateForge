...
def build_v0325_patch_contract(*, out_dir: str = str(DEFAULT_PATCH_CONTRACT_OUT_DIR)) -> dict:
    payload = {
        "schema_version": SCHEMA_VERSION,
        "generated_at_utc": now_utc(),
        "status": "PASS",
        "version_focus": "stage2_local_interface_discovery_coverage",
        ...
        "claim_boundary": "This contract targets widened local interface discovery only. It does not establish open-world interface discovery or topology-heavy structural repair.",
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the v0.3.25 patch contract.")
    parser.add_argument("--out-dir", default=str(DEFAULT_PATCH_CONTRACT_OUT_DIR))
    args = parser.parse_args()
    payload = build_v0325_patch_contract(out_dir=str(args.out_dir))
