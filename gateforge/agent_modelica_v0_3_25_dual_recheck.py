...
def build_v0325_dual_recheck(
    *,
    ...
    if not Path(taskset_path).exists():
        build_v0325_taskset(out_dir=str(Path(taskset_path).parent))
    if not Path(first_fix_summary_path).exists():
        build_v0325_first_fix_evidence(taskset_path=taskset_path, out_dir=str(Path(first_fix_summary_path).parent))
    ...
    if first_fix_ready:
        for task in dual_tasks:
            initial = run_synthetic_task_live(
                ...
                evaluation_label="v0325_dual_initial",
                max_rounds=1,
            ...
            after_first = rerun_once(
                ...
                result_dir=results_dir,
                evaluation_label="v0325_dual_after_first_fix",
                timeout_sec=timeout_sec,
            ...
            if second_residual_exposed and len(task.get("repair_steps") or []) >= 2:
                ...
                final = rerun_once(
                    ...
                    result_dir=results_dir,
                    evaluation_label="v0325_dual_after_second_fix",
                    timeout_sec=timeout_sec,
                )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the v0.3.25 widened dual recheck.")
    ...
    payload = build_v0325_dual_recheck(
