...
def build_v0322_coverage_manifest(*, out_dir: str = str(DEFAULT_MANIFEST_OUT_DIR)) -> dict:
    sources = build_v0322_source_specs()
    single_rows = [dict(row) for row in SINGLE_MISMATCH_SPECS]
    dual_rows = [dict(row) for row in DUAL_MISMATCH_SPECS]
    summary = {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the v0.3.22 coverage expansion manifest.")
    parser.add_argument("--out-dir", default=str(DEFAULT_MANIFEST_OUT_DIR))
    args = parser.parse_args()
    payload = build_v0322_coverage_manifest(out_dir=str(args.out_dir))
