...
from .agent_modelica_v0_10_7_bounded_real_origin_step_worth_it_summary import (
...
from .agent_modelica_v0_10_7_handoff_integrity import build_v107_handoff_integrity
from .agent_modelica_v0_10_7_remaining_uncertainty_characterization import (
    build_v107_remaining_uncertainty_characterization,
)


def build_v107_closeout(
    *,
    ...
    bounded_real_origin_step_worth_it_summary_path: str = str(
        DEFAULT_BOUNDED_REAL_ORIGIN_STEP_WORTH_IT_SUMMARY_OUT_DIR / "summary.json"
    ),
    v106_closeout_path: str = str(DEFAULT_V106_CLOSEOUT_PATH),
    out_dir: str = str(DEFAULT_CLOSEOUT_OUT_DIR),
) -> dict:
    build_v107_handoff_integrity(
    ...
    if integrity.get("handoff_integrity_status") != "PASS":
        payload = {
            ...
            "status": "FAIL",
            "closeout_status": "V0_10_7_REAL_ORIGIN_NEXT_STEP_INPUTS_INVALID",
            "conclusion": {
                "version_decision": "v0_10_7_real_origin_next_step_inputs_invalid",
                "v0_10_8_handoff_mode": "rebuild_v0_10_7_inputs_first",
            },
            "handoff_integrity": integrity,
        }
        out_root = Path(out_dir)
        write_json(out_root / "summary.json", payload)
        write_text(out_root / "summary.md", "# v0.10.7 Closeout\n\n- version_decision: `v0_10_7_real_origin_next_step_inputs_invalid`\n")
        return payload
    ...
    if not Path(bounded_real_origin_step_worth_it_summary_path).exists():
        build_v107_bounded_real_origin_step_worth_it_summary(
            remaining_uncertainty_characterization_path=remaining_uncertainty_characterization_path,
            out_dir=str(Path(bounded_real_origin_step_worth_it_summary_path).parent),
        )

    uncertainty = load_json(remaining_uncertainty_characterization_path)
    summary = load_json(bounded_real_origin_step_worth_it_summary_path)

    worth_it_status = str(summary.get("bounded_real_origin_step_worth_it_status") or "")
    ...
    justified = (
        worth_it_status == "one_more_bounded_real_origin_step_justified"
    ...
    invalid = any(
        [
            ...
            not str(summary.get("bounded_real_origin_step_worth_it_status") or ""),
            uncertainty.get("expected_information_gain") != summary.get("expected_information_gain"),
        ]
    )

    if invalid:
        decision = "v0_10_7_real_origin_next_step_inputs_invalid"
        handoff = "rebuild_v0_10_7_inputs_first"
        status = "FAIL"
        closeout_status = "V0_10_7_REAL_ORIGIN_NEXT_STEP_INPUTS_INVALID"
    elif justified:
        decision = "v0_10_7_one_more_bounded_real_origin_step_justified"
        handoff = "execute_one_more_bounded_real_origin_step_before_phase_synthesis"
        status = "PASS"
        closeout_status = "V0_10_7_ONE_MORE_BOUNDED_REAL_ORIGIN_STEP_JUSTIFIED"
    else:
        decision = "v0_10_7_more_bounded_real_origin_step_not_worth_it"
        handoff = "prepare_v0_10_phase_synthesis"
        status = "PASS"
        closeout_status = "V0_10_7_MORE_BOUNDED_REAL_ORIGIN_STEP_NOT_WORTH_IT"
    ...
    payload = {
        ...
        "conclusion": {
            ...
            "v0_10_8_handoff_mode": handoff,
        },
        "handoff_integrity": integrity,
        "remaining_uncertainty_characterization": uncertainty,
        "bounded_real_origin_step_worth_it_summary": summary,
    }
    out_root = Path(out_dir)
    write_json(out_root / "summary.json", payload)
    write_text(
        ...
        "\n".join(
            [
                ...
                f"- v0_10_8_handoff_mode: `{handoff}`",
            ]
        ),
    )
    return payload
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the v0.10.7 closeout.")
    ...
    parser.add_argument(
        "--bounded-real-origin-step-worth-it-summary",
        default=str(DEFAULT_BOUNDED_REAL_ORIGIN_STEP_WORTH_IT_SUMMARY_OUT_DIR / "summary.json"),
    )
    ...
    payload = build_v107_closeout(
        ...
        bounded_real_origin_step_worth_it_summary_path=str(args.bounded_real_origin_step_worth_it_summary),
