...
def build_v107_bounded_real_origin_step_worth_it_summary(
    *,
    remaining_uncertainty_characterization_path: str,
    out_dir: str = str(DEFAULT_BOUNDED_REAL_ORIGIN_STEP_WORTH_IT_SUMMARY_OUT_DIR),
) -> dict:
    ...
    justified = (
        ...
        and candidate_shape == BOUND_STEP_KIND_TARGETED_NON_SUCCESS_FAMILY_CLARIFICATION
    )
    ...
    if justified:
        ...
        why = (
            "One more bounded real-origin step is justified because the remaining uncertainty is still phase-relevant, "
            "step-addressable, and concentrated enough to name a concrete clarification step without reopening source "
            "expansion, substrate composition, or threshold design."
        )
    else:
        worth_it_status = "more_bounded_real_origin_step_not_worth_it"
        proposed_next_step_kind = BOUND_STEP_KIND_NONE
        why = (
            "One more bounded real-origin step is not worth it because the current real-origin adjudication already "
    ...
    payload = {
        "schema_version": f"{SCHEMA_PREFIX}_bounded_real_origin_step_worth_it_summary",
        "generated_at_utc": now_utc(),
        "status": "PASS",
        "bounded_real_origin_step_worth_it_status": worth_it_status,
    ...
    write_text(
        ...
        "\n".join(
            [
                "# v0.10.7 Bounded Real-Origin Step Worth-It Summary",
                "",
                f"- bounded_real_origin_step_worth_it_status: `{worth_it_status}`",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the v0.10.7 bounded real-origin step worth-it summary.")
    parser.add_argument("--remaining-uncertainty-characterization", required=True)
    parser.add_argument("--out-dir", default=str(DEFAULT_BOUNDED_REAL_ORIGIN_STEP_WORTH_IT_SUMMARY_OUT_DIR))
    ...
    print(json.dumps({"status": payload.get("status"), "bounded_real_origin_step_worth_it_status": payload.get("bounded_real_origin_step_worth_it_status")}))
    return 0
