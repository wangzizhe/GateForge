...
def load_source_blind_multistep_manifest(path: str) -> dict:
    payload = json.loads(Path(path).read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError("source-blind multi-step manifest must be a JSON object")
    payload["_manifest_path"] = str(Path(path).resolve())
    return payload


def validate_source_blind_multistep_manifest(payload: dict) -> tuple[list[dict], list[str]]:
    reasons: list[str] = []
