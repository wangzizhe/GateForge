...
DEFAULT_OUT_DIR = REPO_ROOT / "artifacts" / "residual_consistency_live_attribution_v0_35_21"
...
def _case_row(row: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "residual_consistency_call_count": _tool_call_count(row, "residual_hypothesis_consistency_check"),
        "hypothesis_call_count": _tool_call_count(row, "record_repair_hypothesis"),
    }


def build_residual_consistency_live_attribution(
    *,
    ...
    consistency_tool_used_count = sum(1 for case in cases if case["residual_consistency_call_count"] > 0)
    if not rows:
        decision = "residual_consistency_live_run_missing"
    elif pass_count:
        decision = "residual_consistency_helped_sem22_pass"
    elif success_candidate_seen_count:
        decision = "residual_consistency_found_success_candidate_without_submit"
    elif consistency_tool_used_count:
        decision = "residual_consistency_discoverable_but_no_sem22_pass"
    else:
        decision = "residual_consistency_not_used"
    summary = {
        "version": "v0.35.21",
        "status": "PASS" if rows else "REVIEW",
        "analysis_scope": "residual_consistency_live_attribution",
        "target_case_id": target_case_id,
        "case_count": len(cases),
        "pass_count": pass_count,
        "success_candidate_seen_count": success_candidate_seen_count,
        "consistency_tool_used_count": consistency_tool_used_count,
        "cases": cases,
        "decision": decision,
        "discipline": {
