...
def build_v0333_surface_export_audit(
    *,
    ...
    if not Path(manifest_path).exists():
        build_v0333_coverage_manifest(out_dir=str(Path(manifest_path).parent), use_fixture_only=use_fixture_only)
    ...
    excluded_ids = {norm(row.get("task_id")) for row in export_excluded_rows + canonical_excluded_rows}
    active_single = [task for task in single_rows if norm(task.get("task_id")) not in excluded_ids]
    ...
    summary = {
        ...
        "canonical_miss_task_count": len(canonical_excluded_rows),
        "canonical_miss_task_ids": [norm(row.get("task_id")) for row in canonical_excluded_rows],
        "export_or_surface_substrate_failure_counts": {
            "source_export_missing": len(export_excluded_rows),
            "canonical_missing_from_candidate_set": len(canonical_excluded_rows),
        },
        "active_context_counts": _count_by(active_single, "pipe_slice_context"),
    }
    out_root = Path(out_dir)
    write_json(out_root / "summary.json", summary)
    write_json(out_root / "surface_index.json", {"summary": summary, "task_rows": surface_rows})
    write_json(out_root / "active_taskset.json", {"summary": summary, "single_tasks": active_single, "dual_tasks": active_dual})
    write_text(
        out_root / "summary.md",
        "\n".join(
            [
                "# v0.3.33 Surface Export Audit",
                "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the v0.3.33 widened pipe-slice surface export audit.")
    ...
    payload = build_v0333_surface_export_audit(
