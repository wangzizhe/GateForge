...
SCHEMA_VERSION = "agent_modelica_underconstrained_fast_check_v1"
...
def _write_md(path: str, payload: dict) -> None:
    ...
    p.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# Agent Modelica Underconstrained Fast Check v1",
        "",
...
def _check_ok_from_output(output: str) -> bool:
    ...
    if structural_mismatch:
        try:
            ...
            structural_balance_ok = True
    return "check of" in lower and "completed successfully" in lower and structural_balance_ok


def _check_model_once(*, model_path: str, backend: str, docker_image: str, timeout_sec: int) -> dict:
...
def build_underconstrained_fast_check_v1(
    *,
    ...
    tasks = [
        row
        for row in tasks
        if isinstance(row, dict) and str(row.get("failure_type") or "").strip().lower() == "underconstrained_system"
    ]
    check_runner = runner or (lambda task: _check_model_once(
        model_path=str(task.get("mutated_model_path") or ""),
        backend=backend,
    ...
    for task in tasks:
        ...
        run = check_runner(task)
        diagnostic = run.get("diagnostic_ir") if isinstance(run.get("diagnostic_ir"), dict) else {}
        ...
        blocked_reason = _blocked_reason(str(run.get("output") or ""))
        if stage_match:
            stage_match_count += 1
        check_pass = bool(run.get("check_model_pass"))
        if blocked_reason:
            blocked_count += 1
            reasons.append(f"blocked:{blocked_reason}:{task_id}")
        elif not check_pass and error_type == "model_check_error" and error_subtype == "underconstrained_system" and stage_match:
            pass_count += 1
        else:
    ...
    if total_tasks <= 0:
        status = "FAIL"
        reasons.append("no_underconstrained_tasks")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run check-only fast check for underconstrained realism tasks")
    ...
    parser.add_argument("--out", default="artifacts/agent_modelica_underconstrained_fast_check_v1/summary.json")
    parser.add_argument("--report-out", default="")
    args = parser.parse_args()

    summary = build_underconstrained_fast_check_v1(
