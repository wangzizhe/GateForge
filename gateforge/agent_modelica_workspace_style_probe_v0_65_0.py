...
WORKSPACE_TOOL_DEFS: list[dict[str, Any]] = [
    ...
    {
        "name": "check_candidate_model",
        "description": "Run OMC check/sim preflight on a previously written candidate file.",
        "parameters": {
            "type": "object",
    ...
    },
    {
        ...
        "description": "Run OMC simulation on a previously written candidate file.",
        "parameters": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "read_candidate_model",
        "description": "Read a previously written candidate model from the transparent workspace.",
        "parameters": {
            "type": "object",
            "properties": {"candidate_id": {"type": "string"}},
            "required": ["candidate_id"],
    ...
    },
    {
        ...
        "description": (
            "Submit a previously written candidate model. The wrapper will only evaluate the submitted file; "
...
def _dispatch_workspace_tool(
    *,
    ...
    if name == "read_candidate_model":
        return path.read_text(encoding="utf-8")
    if name in {"check_candidate_model", "simulate_candidate_model"}:
        _, output, check_ok, simulate_ok = _run_omc_on_file(
            workspace=workspace,
            candidate_path=path,
            stop_time=float(arguments.get("stop_time") or 0.05),
            intervals=max(1, int(arguments.get("intervals") or 100)),
        )
        candidate_meta.setdefault(candidate_id, {})["last_check_ok"] = bool(check_ok)
        candidate_meta.setdefault(candidate_id, {})["last_simulate_ok"] = bool(simulate_ok)
...
def run_workspace_style_case(
    ...
    *,
    ...
    system_prompt = (
        "You are fixing a Modelica model in a transparent file workspace.\n"
        "Write each complete candidate with write_candidate_model before checking it.\n"
        "Use check_candidate_model and simulate_candidate_model on candidate files.\n"
        "When one candidate is correct, call submit_candidate_model with its candidate_id.\n"
...
def _candidate_file_audit(case_workspace: Path, *, exclude_stems: set[str] | None = None) -> list[dict[str, Any]]:
...
def _timeout_result(case: dict[str, Any], *, timeout_sec: int, out_dir: Path | None = None) -> dict[str, Any]:
    case_workspace = (out_dir / "workspaces" / str(case.get("case_id") or "")).resolve() if out_dir else Path()
...
def run_workspace_style_probe(
    *,
    ...
    (out_dir / "summary.json").write_text(
        json.dumps(_build_summary(tasks=tasks, results=[], summary_version=summary_version), indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    for task in tasks:
        ...
        (out_dir / "summary.json").write_text(
            json.dumps(_build_summary(tasks=tasks, results=results, summary_version=summary_version), indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
    if not tasks:
        (out_dir / "summary.json").write_text(
            json.dumps(_build_summary(tasks=[], results=[], summary_version=summary_version), indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
    return load_json(out_dir / "summary.json")


def _build_summary(*, tasks: list[dict[str, Any]], results: list[dict[str, Any]], summary_version: str = "v0.66.0") -> dict[str, Any]:
    ...
    return {
        ...
        "analysis_scope": "workspace_style_probe",
