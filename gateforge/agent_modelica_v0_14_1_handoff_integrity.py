...
def build_v141_handoff_integrity(
    *,
    v140_closeout_path: str = str(DEFAULT_V140_CLOSEOUT_PATH),
    out_dir: str = str(DEFAULT_HANDOFF_INTEGRITY_OUT_DIR),
) -> dict:
    out_root = Path(out_dir)
    v140 = load_json(v140_closeout_path)
    conclusion = v140.get("conclusion") if isinstance(v140.get("conclusion"), dict) else {}
    governance_pack = v140.get("governance_pack") if isinstance(v140.get("governance_pack"), dict) else {}
    ...
    upstream_named_pack_ready = bool(conclusion.get("named_first_broader_change_pack_ready"))
    upstream_handoff_mode = str(conclusion.get("v0_14_1_handoff_mode") or "")
    ...
    admitted_ids = frozenset(str(row.get("candidate_id") or "") for row in admitted_rows if isinstance(row, dict))

    version_decision_ok = upstream_version_decision == EXPECTED_V140_VERSION_DECISION
    handoff_mode_ok = upstream_handoff_mode == EXPECTED_V140_HANDOFF_MODE
    ...
    admitted_ids_ok = admitted_ids == EXPECTED_ADMITTED_BROADER_CHANGE_IDS
    ...
    payload = {
        ...
        "upstream_named_first_broader_change_pack_ready": upstream_named_pack_ready,
        "upstream_v0_14_1_handoff_mode": upstream_handoff_mode,
        ...
        "checks": {
            ...
            "named_first_broader_change_pack_ready_ok": named_pack_ready_ok,
            "admitted_candidate_ids_ok": admitted_ids_ok,
        },
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the v0.14.1 handoff integrity artifact.")
    ...
    payload = build_v141_handoff_integrity(
