...
def build_v0324_taskset(
    *,
    ...
    surface_index = surface_index_payload.get("surface_records") if isinstance(surface_index_payload.get("surface_records"), dict) else {}
    ...
    for task in single_candidates:
        if not list(task.get("candidate_symbols") or []):
            export_failures.append({"task_id": norm(task.get("task_id")), "reason": "empty_candidate_surface"})
            continue
        row = dict(task)
    ...
    summary = {
        ...
        "status": "PASS" if execution_mode in {"promoted", "degraded"} and len(active_single) >= 6 and len(active_dual) >= 6 else "FAIL",
        "execution_mode": execution_mode,
        "source_count": len(build_v0324_source_specs()),
        "source_tier_counts": _count_by(build_v0324_source_specs(), "complexity_tier"),
        "single_candidate_task_count": len(single_candidates),
        "dual_candidate_task_count": len(dual_candidates),
        ...
        "export_excluded_family_mix": _count_by(export_failures, "reason"),
        ...
        "surface_record_count": len(surface_index),
    }
    payload = {
        "schema_version": SCHEMA_VERSION,
        "generated_at_utc": now_utc(),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the v0.3.24 local-interface discovery taskset.")
    ...
    payload = build_v0324_taskset(surface_index_path=str(args.surface_index), out_dir=str(args.out_dir), use_fixture_only=bool(args.fixture_only))
