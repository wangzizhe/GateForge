...
from .agent_modelica_v0_3_28_coverage_manifest import build_v0328_coverage_manifest
from .agent_modelica_v0_3_28_dual_recheck import build_v0328_dual_recheck
from .agent_modelica_v0_3_28_first_fix_evidence import build_v0328_first_fix_evidence
from .agent_modelica_v0_3_28_patch_contract import build_v0328_patch_contract
from .agent_modelica_v0_3_28_surface_export_audit import build_v0328_surface_export_audit


SCHEMA_VERSION = f"{SCHEMA_PREFIX}_closeout"


def build_v0328_closeout(
    *,
    ...
    if not Path(manifest_path).exists():
        build_v0328_coverage_manifest(out_dir=str(Path(manifest_path).parent))
    if not Path(surface_audit_path).exists():
        build_v0328_surface_export_audit(out_dir=str(Path(surface_audit_path).parent))
    if not Path(patch_contract_path).exists():
        build_v0328_patch_contract(out_dir=str(Path(patch_contract_path).parent))
    if not Path(first_fix_path).exists():
        build_v0328_first_fix_evidence(out_dir=str(Path(first_fix_path).parent))
    if not Path(dual_recheck_path).exists():
        build_v0328_dual_recheck(out_dir=str(Path(dual_recheck_path).parent))
    ...
    active_single_task_count = int(surface.get("active_single_task_count") or 0)
    active_dual_task_count = int(surface.get("active_dual_sidecar_task_count") or 0)
    if active_single_task_count >= 28 and active_dual_task_count >= 16:
        coverage_construction_mode = "promoted"
    elif active_single_task_count >= 20 and active_dual_task_count >= 12:
        coverage_construction_mode = "degraded_but_executable"
    else:
        coverage_construction_mode = "blocked"

    export_ready = (
        float(surface.get("surface_export_success_rate_pct") or 0.0) >= 80.0
        and float(surface.get("fixture_fallback_rate_pct") or 0.0) <= 0.0
        and float(surface.get("neighbor_post_first_fix_target_bucket_hit_rate_pct") or 0.0) >= 80.0
        and coverage_construction_mode in {"promoted", "degraded_but_executable"}
    )
    boundary_rejected = (
    ...
    if boundary_rejected:
        ...
        version_decision = "stage2_neighbor_component_local_interface_discovery_coverage_not_ready"
    dual_task_count = int(dual.get("task_count") or 0)
    if dual_task_count >= 14 and float(dual.get("neighbor_component_dual_full_resolution_rate_pct") or 0.0) >= 40.0:
    ...
    payload = {
        ...
        "status": "PASS",
        "closeout_status": "STAGE2_NEIGHBOR_COMPONENT_LOCAL_INTERFACE_DISCOVERY_COVERAGE_CLOSEOUT_READY",
        "coverage_manifest": {
        ...
        "surface_export_audit": {
            ...
            "active_dual_sidecar_task_count": active_dual_task_count,
            "coverage_construction_mode": coverage_construction_mode,
        ...
        "first_fix_evidence": {
            ...
            "signal_port_top1_canonical_rate_pct": float(first_fix.get("signal_port_top1_canonical_rate_pct") or 0.0),
            "connector_side_top1_canonical_rate_pct": float(first_fix.get("connector_side_top1_canonical_rate_pct") or 0.0),
        ...
        "dual_recheck": {
            ...
            "signal_port_neighbor_second_residual_rate_pct": float(dual.get("signal_port_neighbor_second_residual_rate_pct") or 0.0),
            "signal_port_neighbor_dual_full_resolution_rate_pct": float(dual.get("signal_port_neighbor_dual_full_resolution_rate_pct") or 0.0),
            "connector_side_neighbor_second_residual_rate_pct": float(dual.get("connector_side_neighbor_second_residual_rate_pct") or 0.0),
            "connector_side_neighbor_dual_full_resolution_rate_pct": float(dual.get("connector_side_neighbor_dual_full_resolution_rate_pct") or 0.0),
        ...
        "conclusion": {
            ...
            "summary": (
                "Widened neighbor-component local interface discovery remains stable across export, first-fix, and dual retention, and the dual denominator is now large enough to upgrade the result beyond directional-only."
                if version_decision == "stage2_neighbor_component_local_interface_discovery_coverage_ready"
                else (
                    "The widened neighbor-component lane remains executable, but one downstream layer or subtype drops before the coverage expansion can be fully promoted."
                    if version_decision == "stage2_neighbor_component_local_interface_discovery_coverage_partially_ready"
                    else (
                        "The widened neighbor-component family crossed into topology-heavy or compile-failure drift; the next version should retreat to a narrower cross-component subtype."
                        if version_decision == "stage2_neighbor_component_local_interface_family_boundary_rejected"
                        else "The widened neighbor-component local interface discovery lane does not yet hold once the slice is expanded."
                    )
                )
            ),
        },
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the v0.3.28 closeout.")
