...
from .agent_modelica_v0_3_27_taskset import build_v0327_taskset
...
def build_v0327_dual_recheck(
    *,
    ...
    if not Path(taskset_path).exists():
        build_v0327_taskset(out_dir=str(Path(taskset_path).parent))
    if not Path(first_fix_summary_path).exists():
        build_v0327_first_fix_evidence(taskset_path=taskset_path, out_dir=str(Path(first_fix_summary_path).parent))
    ...
    if first_fix_ready:
        for task in dual_tasks:
            initial = run_synthetic_task_live(
                ...
                evaluation_label="v0327_neighbor_dual_initial",
                max_rounds=1,
            ...
            after_first = (
                rerun_once(
                    ...
                    result_dir=results_dir,
                    evaluation_label="v0327_neighbor_dual_after_first_fix",
                    timeout_sec=timeout_sec,
                )
            ...
            if second_residual_exposed and len(task.get("repair_steps") or []) >= 2:
                ...
                final = rerun_once(
                    ...
                    result_dir=results_dir,
                    evaluation_label="v0327_neighbor_dual_after_second_fix",
                    timeout_sec=timeout_sec,
                )
    ...
    neighbor_exposed = sum(1 for row in neighbor_rows if bool(row.get("second_residual_exposed")))
    neighbor_retained = sum(1 for row in neighbor_rows if bool(row.get("second_residual_local_interface_retained")))
    neighbor_resolved = sum(1 for row in neighbor_rows if bool(row.get("full_dual_resolution")))
    summary = {
        ...
        "neighbor_component_second_residual_rate_pct": round(100.0 * neighbor_exposed / float(neighbor_count), 1) if neighbor_count else 0.0,
        "neighbor_component_second_residual_local_interface_retained_rate_pct": round(100.0 * neighbor_retained / float(neighbor_count), 1) if neighbor_count else 0.0,
        "neighbor_component_dual_full_resolution_rate_pct": round(100.0 * neighbor_resolved / float(neighbor_count), 1) if neighbor_count else 0.0,
        "rows": rows,
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the v0.3.27 neighbor-component local-interface dual recheck.")
    ...
    payload = build_v0327_dual_recheck(
