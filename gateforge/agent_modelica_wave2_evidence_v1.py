...
SCHEMA_VERSION = "agent_modelica_wave2_evidence_v1"
...
def _decision_status(baseline_pct: float, deterministic_pct: float, retrieval_pct: float) -> tuple[str, str]:
    ...
    return "coverage_high_uplift_zero", "review"


def main() -> None:
    parser = argparse.ArgumentParser(description="Summarize wave2 baseline/deterministic/retrieval evidence")
    ...
    parser.add_argument("--out", default="artifacts/agent_modelica_wave2_realism_live_evidence_v1/evidence_summary.json")
    parser.add_argument("--gate-out", default="artifacts/agent_modelica_wave2_realism_live_evidence_v1/gate_summary.json")
    parser.add_argument("--decision-out", default="artifacts/agent_modelica_wave2_realism_live_evidence_v1/decision_summary.json")
    ...
    if uplift_status == "coverage_high_uplift_zero":
        ...
        if hardest:
            ...
            if hardest_type == "overconstrained_system":
                next_priority_gap_family = "deterministic repair policy"
            elif hardest_type == "parameter_binding_error":
                next_priority_gap_family = "component semantics"
            else:
                next_priority_gap_family = "diagnostic grammar"
