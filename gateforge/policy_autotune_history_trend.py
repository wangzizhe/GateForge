...
def _write_markdown(path: str, payload: dict) -> None:
    ...
    lines = [
        "# GateForge Policy Auto-Tune History Trend",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare policy autotune history summaries and emit trend deltas")
    parser.add_argument("--current", required=True)
    parser.add_argument("--previous", required=True)
    parser.add_argument("--out", default="artifacts/policy_autotune_history/trend.json")
    ...
    d_strict = round(
        _to_float(current.get("strict_suggestion_rate")) - _to_float(previous.get("strict_suggestion_rate")), 4
    )
