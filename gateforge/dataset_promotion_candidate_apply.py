...
APPROVAL_POLICY_DIR = Path("policies/dataset_promotion_apply")
...
def _write_markdown(path: str, summary: dict) -> None:
    ...
    p.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# GateForge Dataset Promotion Candidate Apply",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Apply dataset promotion candidate advisor recommendation with approval")
    parser.add_argument("--advisor-summary", required=True, help="Path to dataset promotion candidate advisor JSON")
    parser.add_argument("--approval", default=None, help="Approval decision JSON path")
    parser.add_argument("--approval-profile", default=None, help="Approval profile under policies/dataset_promotion_apply")
    parser.add_argument("--approval-policy-path", default=None, help="Approval policy path override")
    parser.add_argument("--target-state", default="artifacts/dataset_promotion_candidate/active_promotion.json")
    parser.add_argument("--apply", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--out", default="artifacts/dataset_promotion_candidate/apply_summary.json")
    ...
    confidence_min = _to_float(policy_payload.get("min_confidence_to_apply", 0.7))
    allow_promote_only = bool(policy_payload.get("allow_promote_only", True))
    ...
    if not decision or not action:
        final_status = "FAIL"
        apply_action = "block"
        reasons.append("advisor_payload_missing_decision")
    elif allow_promote_only and decision != "PROMOTE":
        reasons.append("decision_not_promote")
        ...
        state_payload = {
            "active_dataset_promotion_decision": decision,
            "active_dataset_promotion_action": action,
            "source": "dataset_promotion_candidate_apply",
    ...
    print(json.dumps({"final_status": final_status, "apply_action": apply_action, "decision": decision}))
