...
from .agent_modelica_v0_3_26_taskset import build_v0326_taskset
...
def build_v0326_dual_recheck(
    *,
    ...
    if not Path(taskset_path).exists():
        build_v0326_taskset(out_dir=str(Path(taskset_path).parent))
    if not Path(first_fix_summary_path).exists():
        build_v0326_first_fix_evidence(taskset_path=taskset_path, out_dir=str(Path(first_fix_summary_path).parent))
    ...
    if first_fix_ready:
        for task in dual_tasks:
            initial = run_synthetic_task_live(
                ...
                evaluation_label="v0326_dual_initial",
                max_rounds=1,
            ...
            after_first = (
                rerun_once(
                    ...
                    result_dir=results_dir,
                    evaluation_label="v0326_dual_after_first_fix",
                    timeout_sec=timeout_sec,
                )
            ...
            if second_residual_exposed and len(task.get("repair_steps") or []) >= 2:
                ...
                final = rerun_once(
                    ...
                    result_dir=results_dir,
                    evaluation_label="v0326_dual_after_second_fix",
                    timeout_sec=timeout_sec,
                )
    ...
    same_retained = sum(1 for row in same_cluster_rows if bool(row.get("second_residual_local_interface_retained")))
    summary = {
        ...
        "same_cluster_second_residual_local_interface_retained_rate_pct": round(100.0 * same_retained / float(same_count), 1) if same_count else 0.0,
        "dual_full_resolution_rate_pct": round(100.0 * sum(1 for row in rows if bool(row.get("full_dual_resolution"))) / float(len(rows)), 1) if rows else 0.0,
        "rows": rows,
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the v0.3.26 component-type local-interface dual recheck.")
    ...
    payload = build_v0326_dual_recheck(
