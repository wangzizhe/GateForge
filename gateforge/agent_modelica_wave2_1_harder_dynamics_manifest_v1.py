...
def load_wave2_1_harder_dynamics_manifest(path: str) -> dict:
    payload = json.loads(Path(path).read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError("wave2.1 harder-dynamics manifest must be a JSON object")
    payload["_manifest_path"] = str(Path(path).resolve())
    return payload


def validate_wave2_1_harder_dynamics_manifest(payload: dict) -> tuple[list[dict], list[str]]:
    reasons: list[str] = []
    ...
    for idx, library in enumerate(libraries):
        ...
        for model in models:
            ...
            model_count += 1
        if checked_models:
            lib_row = dict(library)
            lib_row["allowed_models"] = checked_models
            validated.append(lib_row)
    if len(validated) < 2:
