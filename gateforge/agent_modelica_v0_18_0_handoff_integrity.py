...
from .agent_modelica_v0_18_0_common import (
    ...
    EXPECTED_V171_PHASE_STOP_CONDITION_STATUS,
    EXPECTED_V171_VERSION_DECISION,
    SCHEMA_PREFIX,
    load_json,
    now_utc,
    write_json,
...
def build_v180_handoff_integrity(
    *,
    v171_closeout_path: str = str(DEFAULT_V171_CLOSEOUT_PATH),
    out_dir: str = str(DEFAULT_HANDOFF_INTEGRITY_OUT_DIR),
) -> dict:
    conclusion = load_json(v171_closeout_path).get("conclusion", {})
    checks = {
        "version_decision_ok": conclusion.get("version_decision") == EXPECTED_V171_VERSION_DECISION,
        "phase_stop_condition_ok": conclusion.get("phase_stop_condition_status") == EXPECTED_V171_PHASE_STOP_CONDITION_STATUS,
        "explicit_caveat_present_ok": bool(conclusion.get("explicit_caveat_present")),
        "explicit_caveat_label_ok": conclusion.get("explicit_caveat_label") == EXPECTED_V171_CAVEAT,
        "next_primary_phase_question_ok": conclusion.get("next_primary_phase_question") == EXPECTED_V171_NEXT_PRIMARY_QUESTION,
        "no_reopen_rule_ok": bool(conclusion.get("do_not_continue_v0_17_same_transition_question_loop_by_default")),
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the v0.18.0 handoff-integrity artifact.")
