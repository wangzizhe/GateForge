...
def build_v0325_taskset(
    *,
    ...
    if not target_surface_index.exists():
    ...
    summary = {
        ...
        "generated_at_utc": now_utc(),
        "status": "PASS" if execution_mode in {"promoted", "degraded"} and len(active_single) >= 18 and len(active_dual) >= 12 else "FAIL",
        "execution_mode": execution_mode,
        "source_count": len(build_v0325_source_specs()),
        "source_tier_counts": _count_by(build_v0325_source_specs(), "complexity_tier"),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the v0.3.25 widened local-interface discovery taskset.")
    ...
    payload = build_v0325_taskset(surface_index_path=str(args.surface_index), out_dir=str(args.out_dir), use_fixture_only=bool(args.fixture_only))
