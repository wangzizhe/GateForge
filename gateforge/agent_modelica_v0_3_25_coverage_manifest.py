...
def build_v0325_coverage_manifest(*, out_dir: str = str(DEFAULT_MANIFEST_OUT_DIR)) -> dict:
    sources = build_v0325_source_specs()
    single_rows = [dict(row) for row in SINGLE_MISMATCH_SPECS]
    dual_rows = [dict(row) for row in DUAL_RECHECK_SPECS]
    summary = {
        "schema_version": SCHEMA_VERSION,
        "generated_at_utc": now_utc(),
        "status": "PASS" if len(single_rows) >= 18 and len(dual_rows) >= 12 else "FAIL",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the v0.3.25 widened local-interface discovery manifest.")
    parser.add_argument("--out-dir", default=str(DEFAULT_MANIFEST_OUT_DIR))
    args = parser.parse_args()
    payload = build_v0325_coverage_manifest(out_dir=str(args.out_dir))
