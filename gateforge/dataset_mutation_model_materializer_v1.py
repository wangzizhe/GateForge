...
def _build_mutation_block(*, failure_type: str, token: str) -> tuple[list[str], str]:
...
def _materialize_text(source_text: str, *, failure_type: str, token: str) -> tuple[str, str]:
    ...
    block, operator = _build_mutation_block(failure_type=failure_type, token=token)
    injection = "\n" + "\n".join(block) + "\n"
    ...
    for midx, model in enumerate(selected_models, start=1):
        ...
        for fidx, failure_type in enumerate(failure_types, start=1):
            for r in range(per_type):
                ...
                mutated_checksum = ""
                try:
                    mutated_text, resolved_operator = _materialize_text(source_text, failure_type=failure_type, token=token)
