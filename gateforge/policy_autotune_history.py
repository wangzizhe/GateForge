...
def _write_markdown(path: str, payload: dict) -> None:
    ...
    p.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# GateForge Policy Auto-Tune History",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Append policy autotune advisor results to history and summarize")
    parser.add_argument("--record", action="append", default=[], help="Advisor JSON path (repeatable)")
    parser.add_argument("--ledger", default="artifacts/policy_autotune_history/history.jsonl", help="History JSONL path")
    parser.add_argument("--out", default="artifacts/policy_autotune_history/summary.json", help="History summary JSON path")
        ...
        append_rows.append(
            {
                ...
                "require_min_top_score_margin": patch.get("require_min_top_score_margin"),
                "require_min_explanation_quality": patch.get("require_min_explanation_quality"),
                "strict_suggestion": profile == "industrial_strict" or confidence >= args.strict_confidence_threshold,
            }
        )
    if append_rows:
    ...
    avg_confidence = round(sum(float(r.get("confidence", 0.0) or 0.0) for r in rows) / max(1, total), 4)
    ...
    if float(latest.get("confidence", 0.0) or 0.0) < 0.6:
    ...
    summary = {
        ...
        "latest_confidence": float(latest.get("confidence", 0.0) or 0.0),
