...
def load_wave2_realism_manifest(path: str) -> dict:
    payload = json.loads(Path(path).read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError("wave2 realism manifest must be a JSON object")
    payload["_manifest_path"] = str(Path(path).resolve())
    return payload


def validate_wave2_realism_manifest(payload: dict) -> tuple[list[dict], list[str]]:
    reasons: list[str] = []
    ...
    for idx, library in enumerate(libraries):
        ...
        if checked_models:
            ...
            library["allowed_models"] = checked_models
