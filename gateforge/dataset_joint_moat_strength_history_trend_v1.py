...
def _write_markdown(path: str, payload: dict) -> None:
    ...
    lines = [
        "# GateForge Joint Moat Strength History Trend v1",
        "",
        f"- status: `{payload.get('status')}`",
        f"- status_transition: `{trend.get('status_transition')}`",
        f"- delta_moat_strength_score: `{trend.get('delta_moat_strength_score')}`",
        "",
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare joint moat strength history summaries and emit trend")
    parser.add_argument("--current", required=True)
    parser.add_argument("--previous", required=True)
    parser.add_argument("--out", default="artifacts/dataset_joint_moat_strength_history_ledger_v1/trend.json")
    ...
    trend = {
        ...
        "delta_moat_strength_score": round(
            _to_float(current.get("latest_moat_strength_score", 0.0))
            - _to_float(previous.get("latest_moat_strength_score", 0.0)),
            4,
        ),
    }
    ...
    if trend["status_transition"] in {"PASS->NEEDS_REVIEW", "PASS->FAIL", "NEEDS_REVIEW->FAIL"}:
        alerts.append("joint_moat_strength_status_worsened")
    if trend["delta_moat_strength_score"] < 0:
