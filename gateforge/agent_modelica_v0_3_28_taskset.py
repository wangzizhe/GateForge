...
def build_v0328_taskset(
    *,
    ...
    if len(active_single) >= 28 and len(active_dual) >= 16:
        coverage_construction_mode = "promoted"
    elif len(active_single) >= 20 and len(active_dual) >= 12:
    ...
    summary = {
        ...
        "generated_at_utc": now_utc(),
        "status": "PASS" if export_success_rate >= 80.0 and coverage_construction_mode in {"promoted", "degraded_but_executable"} else "FAIL",
        "coverage_construction_mode": coverage_construction_mode,
        "task_construction_feasibility_limited": coverage_construction_mode == "degraded_but_executable",
        "source_count": len(build_v0328_source_specs()),
        "source_tier_counts": _count_by(build_v0328_source_specs(), "complexity_tier"),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the v0.3.28 widened neighbor-component local-interface discovery taskset.")
        ...
        json.dumps(
            {
                ...
                "coverage_construction_mode": summary.get("coverage_construction_mode"),
                "active_single_task_count": summary.get("active_single_task_count"),
                "active_dual_task_count": summary.get("active_dual_task_count"),
            }
        )
    )
    return 0
