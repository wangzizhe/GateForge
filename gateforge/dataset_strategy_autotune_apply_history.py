...
def _write_markdown(path: str, payload: dict) -> None:
    ...
    p.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# GateForge Dataset Strategy Autotune Apply History",
        "",
        f"- total_records: `{payload.get('total_records')}`",
        f"- latest_final_status: `{payload.get('latest_final_status')}`",
        f"- latest_apply_action: `{payload.get('latest_apply_action')}`",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Append dataset strategy apply summaries and emit aggregated history")
    parser.add_argument("--record", action="append", default=[], help="dataset_strategy_autotune_apply summary JSON path")
    parser.add_argument("--ledger", default="artifacts/dataset_strategy_apply_history/history.jsonl", help="History JSONL path")
    parser.add_argument("--out", default="artifacts/dataset_strategy_apply_history/summary.json", help="Output summary JSON path")
    parser.add_argument("--report-out", default=None, help="Output summary markdown path")
        ...
        append_rows.append(
            {
                ...
                "suggested_policy_profile": payload.get("suggested_policy_profile"),
    ...
    for row in rows:
        ...
        action = str(row.get("apply_action") or "unknown")
    ...
    alerts: list[str] = []
    if str(latest.get("final_status") or "") == "NEEDS_REVIEW":
        alerts.append("latest_apply_needs_review")
    ...
    summary = {
        ...
        "latest_suggested_policy_profile": latest.get("suggested_policy_profile"),
