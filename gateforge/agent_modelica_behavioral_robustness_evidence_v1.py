...
def _deterministic_partial_to_full(taskset_payload: dict, baseline_results: dict, deterministic_results: dict) -> tuple[int, dict[str, dict]]:
    ...
    for task_id, det_record in deterministic_map.items():
        ...
        row = by_failure_type.setdefault(
            failure_type,
            {"task_count": 0, "partial_to_full_count": 0},
        )
        row["task_count"] += 1
...
def main() -> None:
    ...
    parser.add_argument("--source-mode", default="source_aware")
    ...
    source_mode = str(args.source_mode or "source_aware").strip().lower() or "source_aware"
    ...
    deterministic_partial_to_full_count, deterministic_partial_to_full_by_failure = _deterministic_partial_to_full(
        taskset,
        baseline_results,
    ...
    for failure_type in sorted(set(baseline_by_failure) | set(deterministic_by_failure) | set(retrieval_by_failure)):
        ...
        partial_to_full = deterministic_partial_to_full_by_failure.get(failure_type, {})
        by_failure_type[failure_type] = {
            ...
            "deterministic_partial_to_full_count": int(partial_to_full.get("partial_to_full_count") or 0),
            "deterministic_partial_to_full_pct": float(partial_to_full.get("partial_to_full_pct") or 0.0),
        }
    ...
    evidence = {
        ...
        "deterministic_partial_to_full_count": deterministic_partial_to_full_count,
        ...
        "deterministic_by_failure_type": by_failure_type,
