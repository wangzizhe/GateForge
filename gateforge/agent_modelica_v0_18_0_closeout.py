...
from .agent_modelica_v0_18_0_governance_pack import build_v180_governance_pack
from .agent_modelica_v0_18_0_handoff_integrity import build_v180_handoff_integrity


def build_v180_closeout(
    *,
    ...
    if handoff.get("handoff_integrity_status") != "PASS":
        payload = {
            ...
            "status": "FAIL",
            "closeout_status": "V0_18_0_HANDOFF_PHASE_INPUTS_INVALID",
            "conclusion": {
                "version_decision": "v0_18_0_handoff_phase_inputs_invalid",
                "next_honest_move_governance_status": "invalid",
                "governance_ready_for_runtime_execution": False,
                "minimum_completion_signal_pass": False,
                "next_move_viability_status": "invalid",
                "next_move_governance_outcome": "invalid",
                "v0_18_1_handoff_mode": "rebuild_v0_18_0_phase_inputs_first",
            },
            "handoff_integrity": handoff,
        }
        write_json(out_root / "summary.json", payload)
        write_text(out_root / "summary.md", "# v0.18.0 Closeout\n\n- version_decision: `v0_18_0_handoff_phase_inputs_invalid`\n")
        return payload
    ...
    governance_status = governance.get("next_honest_move_governance_status", "invalid")
    ...
    named_first_next_move_pack_ready = bool(governance.get("named_first_next_move_pack_ready"))
    viability_object = governance.get("next_move_viability", {})
    viability_status = viability_object.get("next_move_viability_status", "invalid")
    governance_outcome = (governance.get("governance_outcome") or {}).get("next_move_governance_outcome", "invalid")

    if governance_outcome == "no_honest_next_move_remains":
        version_decision = "v0_18_0_no_honest_next_move_remains"
        handoff_mode = "prepare_v0_18_phase_closeout_or_stop"
        status = "PASS"
        why = (
            "The compressed governance layer froze successfully enough to conclude that no honest next move remains beyond the "
            "carried evidence-boundary readout."
        )
    elif minimum_completion_signal_pass and viability_status == "justified" and governance_outcome == "next_honest_move_exists":
        version_decision = "v0_18_0_next_honest_move_governance_ready"
        handoff_mode = "evaluate_first_next_honest_move_or_close_in_same_phase"
        status = "PASS"
        why = (
            "A named next move survived admission, preserved an honest reframing contract, and justified opening the next branch."
        )
    elif handoff.get("handoff_integrity_status") == "PASS":
        version_decision = "v0_18_0_next_honest_move_governance_partial"
        handoff_mode = "resolve_v0_18_0_governance_or_viability_gaps_first"
        status = "PASS"
        why = (
            "The next-move direction is readable, but it has not yet frozen into either a justified next branch or a terminal "
            "no-honest-next-move conclusion."
        )
    else:
        version_decision = "v0_18_0_handoff_phase_inputs_invalid"
        handoff_mode = "rebuild_v0_18_0_phase_inputs_first"
        status = "FAIL"
        why = "The carried handoff or compressed governance layer is invalid."
    ...
    payload = {
        ...
        "conclusion": {
            ...
            "next_move_viability_status": viability_status,
            "next_move_governance_outcome": governance_outcome,
            "why_this_is_or_is_not_ready": why,
            "v0_18_1_handoff_mode": handoff_mode,
        },
        "handoff_integrity": handoff,
    ...
    write_text(
        ...
        "\n".join(
            [
                ...
                f"- next_move_viability_status: `{viability_status}`",
                f"- next_move_governance_outcome: `{governance_outcome}`",
                f"- v0_18_1_handoff_mode: `{handoff_mode}`",
            ]
        ),
    )
    return payload
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the v0.18.0 next-honest-move governance closeout.")
