...
def build_v0326_coverage_manifest(*, out_dir: str = str(DEFAULT_MANIFEST_OUT_DIR)) -> dict:
    sources = build_v0326_source_specs()
    single_rows = [dict(row) for row in SINGLE_MISMATCH_SPECS]
    dual_rows = [dict(row) for row in DUAL_RECHECK_SPECS]
    summary = {
        "schema_version": SCHEMA_VERSION,
        "generated_at_utc": now_utc(),
        "status": "PASS" if len(single_rows) >= 12 and len(dual_rows) >= 10 else "FAIL",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the v0.3.26 component-type local-interface discovery manifest.")
    parser.add_argument("--out-dir", default=str(DEFAULT_MANIFEST_OUT_DIR))
    args = parser.parse_args()
    payload = build_v0326_coverage_manifest(out_dir=str(args.out_dir))
