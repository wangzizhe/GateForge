...
def build_v0320_dual_recheck(
    *,
    ...
    if not Path(first_fix_summary_path).exists():
        build_v0320_first_fix_evidence(out_dir=str(DEFAULT_FIRST_FIX_OUT_DIR))
    ...
    first_fix_ready = (
        float(first_fix_summary.get("patch_applied_rate_pct") or 0.0) >= 70.0
    ...
    if first_fix_ready:
        for task in dual_tasks:
            initial = run_synthetic_task_live(
                ...
                evaluation_label="v0320_dual_initial",
                max_rounds=1,
            ...
            patched_once_text, first_patch_audit = apply_constrained_first_fix(
                ...
                candidate_key=first_step.get("candidate_key_repr"),
            )
            if not first_patch_audit.get("applied"):
                patched_once_text, first_patch_audit = apply_constrained_first_fix(
                    current_text=norm(task.get("mutated_model_text")),
                    patch_type=norm(first_step.get("patch_type")),
                    wrong_symbol=norm(first_step.get("wrong_symbol")),
                    candidate_key=(norm(first_step.get("component_type")), norm(first_step.get("wrong_symbol"))) if norm(first_step.get("patch_type")) == "replace_parameter_name" else norm(first_step.get("wrong_symbol")),
                )
            after_first = rerun_once(
                ...
                result_dir=results_dir,
                evaluation_label="v0320_dual_after_first_fix",
                timeout_sec=timeout_sec,
            ...
            if second_residual_exposed and len(task.get("repair_steps") or []) >= 2:
                ...
                patched_twice_text, second_patch_audit = apply_constrained_first_fix(
                    current_text=patched_once_text,
                    patch_type=norm(second_step.get("patch_type")),
                    wrong_symbol=norm(second_step.get("wrong_symbol")),
                    candidate_key=(norm(second_step.get("component_type")), norm(second_step.get("wrong_symbol"))) if norm(second_step.get("patch_type")) == "replace_parameter_name" else norm(second_step.get("wrong_symbol")),
                )
                final = rerun_once(
    ...
    summary = {
        ...
        "first_fix_execution_ready": first_fix_ready,
        "task_count": len(rows),
    ...
    write_text(
        ...
        "\n".join(
            [
                "# v0.3.20 Dual Recheck",
                "",
                f"- status: `{summary.get('status')}`",
                f"- first_fix_execution_ready: `{summary.get('first_fix_execution_ready')}`",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the v0.3.20 dual-mismatch recheck.")
    ...
    payload = build_v0320_dual_recheck(
