...
- Optional `checkers` field in proposal:
  - `["timeout", "nan_inf", "performance_regression", "event_explosion", "steady_state_regression", "control_behavior_regression", "invariant_guard"]`
...
- Optional `checker_config` field in proposal:
  ...
  - `{"invariant_guard": {"invariants": [{"type":"range","metric":"steady_state_error","min":0.0,"max":0.08}]}}`
  - `{"_runtime": {"enable": ["steady_state_regression"], "disable": ["performance_regression"]}}`
- Optional `physical_invariants` field in proposal:
  - `range`: `{"type":"range","metric":"steady_state_error","min":0.0,"max":0.08}`
  - `monotonic`: `{"type":"monotonic","metric":"energy","direction":"non_increasing"}`
  - `bounded_delta`: `{"type":"bounded_delta","metric":"overshoot","max_abs_delta":0.1}`
  - At runtime these are auto-injected into `checker_config.invariant_guard.invariants`.
...
Invariant-repair profile comparison demo:
...
Or use repair orchestrate compare output as the repair source:
...
Orchestrate-compare governance demo shortcut:
...
Promotion profile compare (single snapshot -> multi-profile ranking):
...
Run full pipeline in one command (`repair_tasks -> repair_pack -> repair_batch`):

```bash
python -m gateforge.repair_orchestrate \
...
Compare two repair strategies in the same orchestrated pipeline:
...
- Provides an optional repair-loop safety guard demo job (`workflow_dispatch` with `run_repair_loop_safety_guard=true`) that does not block the main job.
  This job publishes blocked-repair outcome when new critical reasons appear after a repair attempt.
...
- Provides an optional repair-orchestrate demo job (`workflow_dispatch` with `run_repair_orchestrate_demo=true`) that does not block the main job.
  This job validates the one-command repair pipeline from fail summary to batch results.
- Provides an optional repair-orchestrate-compare demo job (`workflow_dispatch` with `run_repair_orchestrate_compare_demo=true`) that does not block the main job.
  This job validates strategy-profile deltas under one shared failure source.
...
- Provides an optional governance snapshot from orchestrate-compare demo job (shares `workflow_dispatch` toggle `run_governance_snapshot_demo=true`) that does not block the main job.
  This job publishes governance status using `repair_orchestrate` strategy-compare output.
- Provides an optional governance snapshot trend demo job (shares `workflow_dispatch` toggle `run_governance_snapshot_demo=true`) that does not block the main job.
...
- Provides an optional invariant repair profile-compare demo job (`workflow_dispatch` with `run_invariant_repair_profile_compare_demo=true`) that does not block the main job.
  This job publishes decision/confidence deltas between default and industrial invariant-repair profiles.
...
4. Enable `run_benchmark` and/or `run_checker_demo` and/or `run_steady_state_demo` and/or `run_behavior_metrics_demo` and/or `run_demo_bundle` and/or `run_autopilot_dry_run` and/or `run_agent_change_loop` and/or `run_repair_loop` and/or `run_repair_loop_safety_guard` and/or `run_planner_guardrails` and/or `run_planner_output_validate_demo` and/or `run_repair_batch_demo` and/or `run_repair_batch_compare_demo` and/or `run_repair_pack_from_tasks_demo` and/or `run_repair_tasks_demo` and/or `run_repair_orchestrate_demo` and/or `run_repair_orchestrate_compare_demo` and/or `run_governance_snapshot_demo` and/or `run_governance_history_demo` and/or `run_governance_promote_demo` and/or `run_governance_promote_compare_demo` and/or `run_governance_promote_apply_demo` and/or `run_agent_invariant_guard_demo` and/or `run_invariant_repair_loop_demo` and/or `run_invariant_repair_profile_compare_demo`.
...
You can include repair-loop safety guard in local matrix with:
`RUN_REPAIR_LOOP_SAFETY_GUARD=1 bash scripts/demo_ci_matrix.sh`
...
You can include repair orchestrate demo in local matrix with:
`RUN_REPAIR_ORCHESTRATE_DEMO=1 bash scripts/demo_ci_matrix.sh`

You can include repair orchestrate compare demo in local matrix with:
`RUN_REPAIR_ORCHESTRATE_COMPARE_DEMO=1 bash scripts/demo_ci_matrix.sh`
...
You can include governance snapshot orchestrate demo in local matrix with:
`RUN_GOVERNANCE_SNAPSHOT_ORCHESTRATE_DEMO=1 bash scripts/demo_ci_matrix.sh`
...
You can include invariant repair profile-compare demo in local matrix with:
`RUN_INVARIANT_REPAIR_PROFILE_COMPARE_DEMO=1 bash scripts/demo_ci_matrix.sh`
