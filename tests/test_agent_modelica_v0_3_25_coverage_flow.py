...
from gateforge.agent_modelica_v0_3_25_coverage_manifest import build_v0325_coverage_manifest
from gateforge.agent_modelica_v0_3_25_surface_export_audit import build_v0325_surface_export_audit
from gateforge.agent_modelica_v0_3_25_taskset import build_v0325_taskset


class AgentModelicaV0325CoverageFlowTests(unittest.TestCase):
    def test_manifest_and_fixture_surface_audit_freeze_expanded_lane(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            manifest = build_v0325_coverage_manifest(out_dir=str(root / "manifest"))
            audit = build_v0325_surface_export_audit(
            ...
            self.assertGreaterEqual(int(summary.get("single_task_count") or 0), 18)
    ...
    def test_taskset_coverage_counts_expand_beyond_v0324(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            build_v0325_surface_export_audit(
                out_dir=str(root / "audit"),
                surface_index_out_dir=str(root / "surface"),
                taskset_out_dir=str(root / "taskset"),
                use_fixture_only=True,
    ...
    def test_closeout_partial_when_dual_is_bottleneck(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            build_v0325_coverage_manifest(out_dir=str(root / "manifest"))
            ...
            (root / "contract" / "summary.json").write_text(
                json.dumps(
                    {
                        "status": "PASS",
                        "selection_mode": "authoritative_local_interface_surface_only",
            ...
            self.assertEqual(conclusion.get("version_decision"), "stage2_local_interface_discovery_coverage_partially_ready")
