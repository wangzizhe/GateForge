...
class WorkspaceStyleProbeV067Tests(unittest.TestCase):
    def test_tool_count_is_three(self) -> None:
        self.assertEqual(len(WORKSPACE_TOOL_DEFS), 3)
        tool_names = {t["name"] for t in WORKSPACE_TOOL_DEFS}
        self.assertSetEqual(
            tool_names,
            {"write_and_check_candidate_model", "simulate_candidate_model", "submit_candidate_model"},
        )
