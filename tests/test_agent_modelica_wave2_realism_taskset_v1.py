...
def _manifest_payload(root: Path) -> dict:
    ...
    for lib in ("LibA", "LibB", "LibC"):
        for name in ("A", "B"):
            ...
            _write_model(path, name)
            models.append((lib, name, path))
    return {
...
class AgentModelicaWave2RealismTasksetV1Tests(unittest.TestCase):
    def test_builder_produces_wave2_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            out_dir = root / "out"
            proc = subprocess.run(
                [sys.executable, "-m", "gateforge.agent_modelica_wave2_realism_taskset_v1", "--manifest", str(manifest), "--out-dir", str(out_dir)],
                capture_output=True,
                text=True,
                check=False,
            )
            ...
            self.assertEqual(summary.get("counts_by_failure_type"), {"overconstrained_system": 6, "parameter_binding_error": 6, "array_dimension_mismatch": 6})
            self.assertEqual(summary.get("counts_by_error_family"), {"constraint_violation": 6, "model_check_error": 12})
            first = taskset["tasks"][0]
            self.assertIn(first.get("failure_type"), {"overconstrained_system", "parameter_binding_error", "array_dimension_mismatch"})
            self.assertTrue(first.get("expected_observed_error_type"))
