...
class RunPrivateModelMutationScaleBatchV1Tests(unittest.TestCase):
    def test_run_private_batch_script(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertIn(summary.get("mutation_effective_depth_status"), {"PASS", "NEEDS_REVIEW", "FAIL"})
            self.assertIn(summary.get("mutation_effective_depth_history_status"), {"PASS", "NEEDS_REVIEW", "FAIL"})
            self.assertIn(summary.get("mutation_effective_depth_history_trend_status"), {"PASS", "NEEDS_REVIEW", "FAIL"})
            self.assertGreaterEqual(float(summary.get("mutation_effective_depth_p10_effective_mutations_per_model", 0.0) or 0.0), 0.0)
            self.assertIn(summary.get("mutation_source_provenance_status"), {"PASS", "NEEDS_REVIEW", "FAIL"})
            self.assertIn(summary.get("mutation_source_provenance_history_status"), {"PASS", "NEEDS_REVIEW", "FAIL"})
            self.assertIn(summary.get("mutation_source_provenance_history_trend_status"), {"PASS", "NEEDS_REVIEW", "FAIL"})
            self.assertGreaterEqual(float(summary.get("mutation_source_provenance_existing_source_path_ratio_pct", 0.0) or 0.0), 0.0)
            self.assertIn(summary.get("mutation_authentic_scale_score_status"), {"PASS", "NEEDS_REVIEW", "FAIL"})
            self.assertGreaterEqual(float(summary.get("mutation_authentic_scale_score", 0.0) or 0.0), 0.0)
            self.assertIn(summary.get("mutation_authentic_scale_score_history_status"), {"PASS", "NEEDS_REVIEW", "FAIL"})
            self.assertIn(summary.get("mutation_authentic_scale_score_history_trend_status"), {"PASS", "NEEDS_REVIEW", "FAIL"})
            self.assertIn(summary.get("mutation_source_bucket_effective_scale_status"), {"PASS", "NEEDS_REVIEW", "FAIL"})
            self.assertGreaterEqual(int(summary.get("mutation_source_bucket_effective_scale_bucket_count", 0) or 0), 1)
            self.assertIn(summary.get("mutation_source_bucket_effective_scale_history_status"), {"PASS", "NEEDS_REVIEW", "FAIL"})
            self.assertIn(summary.get("mutation_source_bucket_effective_scale_history_trend_status"), {"PASS", "NEEDS_REVIEW", "FAIL"})
            self.assertIn(summary.get("large_model_authenticity_status"), {"PASS", "NEEDS_REVIEW", "FAIL"})
            self.assertGreaterEqual(float(summary.get("large_model_authenticity_score", 0.0) or 0.0), 0.0)
            self.assertIn(summary.get("large_model_authenticity_history_status"), {"PASS", "NEEDS_REVIEW", "FAIL"})
            self.assertIn(summary.get("large_model_authenticity_history_trend_status"), {"PASS", "NEEDS_REVIEW", "FAIL"})
            self.assertIn(summary.get("moat_weekly_target_gate_status"), {"PASS", "NEEDS_REVIEW", "FAIL"})
