...
class RunPrivateModelMutationDepth6SprintV1Tests(unittest.TestCase):
    def test_run_depth6_sprint_with_small_local_pool(self) -> None:
        repo_root = Path(__file__).resolve().parents[1]
        script = repo_root / "scripts" / "run_private_model_mutation_depth6_sprint_v1.sh"
        with tempfile.TemporaryDirectory() as d:
            ...
            baseline.write_text(
                json.dumps(
                    {
                        "generated_mutations": 8,
                        "reproducible_mutations": 8,
                        "mutations_per_failure_type": 4,
                    }
                ),
                encoding="utf-8",
            )
            out_dir = root / "depth6_out"
            proc = subprocess.run(
                ["bash", str(script)],
                cwd=str(repo_root),
                capture_output=True,
                ...
                env={
                    **os.environ,
                    ...
                    "GATEFORGE_MIN_GENERATED_MUTATIONS": "12",
            ...
            self.assertGreaterEqual(int(summary.get("mutations_per_failure_type", 0) or 0), 6)
