...
class DatasetMilestoneCheckpointV1Tests(unittest.TestCase):
    def test_checkpoint_pass_or_review(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            proc = subprocess.run([sys.executable, "-m", "gateforge.dataset_milestone_checkpoint_v1", "--moat-trend-snapshot-summary", str(moat), "--moat-public-scoreboard-summary", str(scoreboard), "--snapshot-moat-alignment-summary", str(alignment), "--modelica-release-candidate-gate-summary", str(release), "--out", str(out)], capture_output=True, text=True, check=False)
