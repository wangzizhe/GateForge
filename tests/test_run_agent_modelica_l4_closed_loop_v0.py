...
def _live_cmd_success_with_timeout_blip() -> str:
    code = """
import json
payload = {
  "check_model_pass": True,
  ...
  "attempts": [
    {
      ...
      "stderr_snippet": "TimeoutError: The read operation timed out",
      "log_excerpt": "TimeoutError: The read operation timed out",
      "diagnostic_ir": {
        "error_type": "none",
        "error_subtype": "none",
        "stage": "none",
...
class RunAgentModelicaL4ClosedLoopV0Tests(unittest.TestCase):
    ...
    def test_script_counts_passed_timeout_attempts_as_blips_not_blocking_infra(self) -> None:
        repo_root = Path(__file__).resolve().parents[1]
        script = repo_root / "scripts" / "run_agent_modelica_l4_closed_loop_v0.sh"
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            taskset = root / "taskset.json"
            out_dir = root / "out_blip"
            model_path = root / "A1.mo"
            model_path.write_text(
                "\n".join(
                    [
            ...
            env = {
                **os.environ,
                "GATEFORGE_AGENT_L4_CLOSED_LOOP_TASKSET": str(taskset),
                "GATEFORGE_AGENT_L4_CLOSED_LOOP_OUT_DIR": str(out_dir),
                "GATEFORGE_AGENT_L4_CLOSED_LOOP_LIVE_EXECUTOR_CMD": _live_cmd_success_with_timeout_blip(),
                "GATEFORGE_AGENT_L4_MIN_SUCCESS_DELTA_PP": "0",
                "GATEFORGE_AGENT_L4_CLOSED_LOOP_MAX_ROUNDS": "1",
                "GATEFORGE_AGENT_L4_MAX_ROUNDS": "2",
                "GATEFORGE_AGENT_L4_CLOSED_LOOP_MAX_TIME_SEC": "20",
                "GATEFORGE_AGENT_L4_CLOSED_LOOP_LIVE_TIMEOUT_SEC": "20",
            ...
            summary = json.loads((out_dir / "ab_compare_summary.json").read_text(encoding="utf-8"))
            on = summary.get("on") if isinstance(summary.get("on"), dict) else {}
            self.assertEqual(int(on.get("infra_failure_count", -1)), 0)
            self.assertEqual(int(on.get("infra_attempt_blip_count", -1)), 1)
            self.assertNotIn("on_infra_failure_present", set(summary.get("reasons") or []))
