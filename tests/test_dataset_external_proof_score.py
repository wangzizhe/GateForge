...
class DatasetExternalProofScoreTests(unittest.TestCase):
    def test_proof_score_outputs_numeric_score(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            proc = subprocess.run([sys.executable, "-m", "gateforge.dataset_external_proof_score", "--evidence-release-manifest", str(m), "--moat-execution-forecast", str(f), "--governance-decision-proofbook", str(p), "--out", str(out)], capture_output=True, text=True, check=False)
