...
class AgentModelicaLiveExecutorGeminiV1Tests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory(prefix="gf_live_exec_docker_cache_") as td:
            ...
            try:
                with patch(
                    ...
                    self.assertIn(f"{str(cache_dir.resolve())}:/workspace/.omc_home/.openmodelica/libraries", cmd)
                    self.assertIn("--user", cmd)
                    self.assertIn("-e", cmd)
                    self.assertIn("HOME=/workspace/.omc_home", cmd)
            finally:
                if prev is None:
    ...
    def test_main_emits_experience_replay_order_when_enabled(self) -> None:
        with tempfile.TemporaryDirectory(prefix="gf_live_exec_replay_") as td:
            ...
            self.assertEqual(int(payload.get("rounds_used") or 0), len(payload.get("attempts") or []))
