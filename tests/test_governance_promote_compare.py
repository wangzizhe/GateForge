...
class GovernancePromoteCompareTests(unittest.TestCase):
    def test_promote_compare_selects_best_profile(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            ranking_details = payload.get("decision_explanation_ranking_details", {})
            self.assertIsInstance(ranking_details, dict)
            self.assertIsInstance(ranking_details.get("top_driver"), str)
            self.assertGreaterEqual(int(ranking_details.get("numeric_reason_count", 0)), 1)
            self.assertIsInstance(ranking_details.get("drivers"), list)
            self.assertGreaterEqual(len(ranking_details.get("drivers", [])), 1)
