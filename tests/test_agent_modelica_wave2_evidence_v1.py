...
class AgentModelicaWave2EvidenceV1Tests(unittest.TestCase):
    def _run(self, root: Path, *, baseline: float, deterministic: float, retrieval: float) -> subprocess.CompletedProcess[str]:
        ...
        _write_json(taskset, {"tasks": [{"task_id": "t1", "failure_type": "overconstrained_system"}]})
        _write_json(challenge, {"taskset_frozen_path": str(taskset), "total_tasks": 1, "counts_by_library": {"liba": 1}, "counts_by_failure_type": {"overconstrained_system": 1}})
        ...
        return subprocess.run(
            [
                sys.executable,
                "-m",
                "gateforge.agent_modelica_wave2_evidence_v1",
                "--challenge-summary",
                str(challenge),
                "--baseline-summary",
    ...
    def test_detects_retrieval_uplift(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            proc = self._run(root, baseline=50.0, deterministic=50.0, retrieval=100.0)
            self.assertEqual(proc.returncode, 0, msg=proc.stderr or proc.stdout)
            decision = json.loads((root / "decision.json").read_text(encoding="utf-8"))
            self.assertEqual(decision.get("retrieval_uplift_status"), "retrieval_uplift_observed")
