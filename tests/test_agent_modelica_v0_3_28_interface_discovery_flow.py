...
from gateforge.agent_modelica_v0_3_28_common import apply_interface_discovery_first_fix, build_surface_index_payload
...
class AgentModelicaV0328InterfaceDiscoveryFlowTests(unittest.TestCase):
    def test_surface_index_uses_component_type_local_candidates_for_neighbor_pairs(self) -> None:
        ...
        self.assertIn("resistor1.posPin", records)
        self.assertIn("resistor1.p", (records.get("resistor1.posPin") or {}).get("candidate_symbols") or [])
    ...
    def test_taskset_and_contract_pass_with_widened_neighbor_counts(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            taskset = build_v0328_taskset(out_dir=str(root / "taskset"), use_fixture_only=True)
            contract = build_v0328_patch_contract(out_dir=str(root / "contract"))
    ...
    def test_closeout_promotes_neighbor_coverage_ready_with_fixture_summaries(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            ...
            (root / "manifest" / "summary.json").write_text(
                json.dumps({"status": "PASS", "source_count": 8, "single_task_count": 38, "dual_sidecar_task_count": 19}),
                encoding="utf-8",
            )
                ...
                json.dumps(
                    {
                        ...
                        "signal_port_neighbor_second_residual_rate_pct": 80.0,
                        "signal_port_neighbor_dual_full_resolution_rate_pct": 70.0,
                        "connector_side_neighbor_second_residual_rate_pct": 80.0,
                        "connector_side_neighbor_dual_full_resolution_rate_pct": 70.0,
                        "second_residual_local_interface_retained_count": 12,
                        "full_dual_resolution_count": 11,
                        "task_count": 16,
                    }
                ),
                encoding="utf-8",
            ...
            self.assertEqual(conclusion.get("version_decision"), "stage2_neighbor_component_local_interface_discovery_coverage_ready")
