...
class RunAgentModelicaHoldoutCheckpointV1Tests(unittest.TestCase):
    def test_script_wires_holdout_builder_and_layered_baseline(self) -> None:
        ...
        self.assertIn("gateforge.agent_modelica_failure_attribution_v1", content)
        self.assertIn("gateforge.agent_modelica_focus_queue_from_attribution_v1", content)
        ...
        self.assertIn('RUN_MODE="${GATEFORGE_AGENT_RUN_MODE:-evidence}"', content)
        self.assertIn("--live-executor-cmd \"$LIVE_EXECUTOR_CMD\"", content)
