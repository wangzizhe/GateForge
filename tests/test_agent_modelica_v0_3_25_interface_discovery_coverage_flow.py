...
from gateforge.agent_modelica_v0_3_25_closeout import build_v0325_closeout
from gateforge.agent_modelica_v0_3_25_common import apply_interface_discovery_first_fix, build_surface_index_payload
from gateforge.agent_modelica_v0_3_25_patch_contract import build_v0325_patch_contract
from gateforge.agent_modelica_v0_3_25_taskset import build_v0325_taskset


class AgentModelicaV0325InterfaceDiscoveryCoverageFlowTests(unittest.TestCase):
    def test_surface_index_builds_widened_local_interface_candidate_sets(self) -> None:
    ...
    def test_interface_discovery_patch_prefers_expected_widened_local_candidate(self) -> None:
    ...
    def test_taskset_and_contract_pass_with_coverage_counts(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            taskset = build_v0325_taskset(out_dir=str(root / "taskset"), use_fixture_only=True)
            contract = build_v0325_patch_contract(out_dir=str(root / "contract"))
    ...
    def test_closeout_promotes_widened_ready_with_fixture_summaries(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
                ...
                json.dumps(
                    {
                        "status": "PASS",
                        "source_count": 5,
                        "single_task_count": 25,
                        "dual_sidecar_task_count": 12,
                    }
                ),
                encoding="utf-8",
            )
            ...
            build_v0325_patch_contract(out_dir=str(root / "contract"))
            ...
            self.assertEqual((closeout.get("conclusion") or {}).get("version_decision"), "stage2_local_interface_discovery_coverage_ready")
