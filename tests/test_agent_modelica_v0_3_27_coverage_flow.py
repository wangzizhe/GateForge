...
from gateforge.agent_modelica_v0_3_27_coverage_manifest import build_v0327_coverage_manifest
from gateforge.agent_modelica_v0_3_27_surface_export_audit import build_v0327_surface_export_audit
from gateforge.agent_modelica_v0_3_27_taskset import build_v0327_taskset


class AgentModelicaV0327CoverageFlowTests(unittest.TestCase):
    def test_manifest_and_fixture_surface_audit_freeze_neighbor_lane(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            manifest = build_v0327_coverage_manifest(out_dir=str(root / "manifest"))
            audit = build_v0327_surface_export_audit(
    ...
    def test_taskset_neighbor_counts_hold_with_precheck(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            build_v0327_surface_export_audit(
                out_dir=str(root / "audit"),
                surface_index_out_dir=str(root / "surface"),
                taskset_out_dir=str(root / "taskset"),
                use_fixture_only=True,
            )
            taskset = build_v0327_taskset(
    ...
    def test_closeout_partial_when_neighbor_dual_is_bottleneck(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            build_v0327_coverage_manifest(out_dir=str(root / "manifest"))
            ...
            self.assertEqual(conclusion.get("version_decision"), "stage2_neighbor_component_local_interface_discovery_partially_ready")
