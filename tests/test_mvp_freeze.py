...
class MVPFreezeTests(unittest.TestCase):
    def _write_artifacts(
        ...
        governance_snapshot_advisor_history = root / "governance_snapshot_advisor_history.json"
        ...
        autotune_gov.write_text(
            json.dumps(
                {
                    "bundle_status": "PASS",
                    "advisor_history_latest_top_driver": "component_delta:recommended_component",
                    "advisor_history_top_driver_non_null_rate": 1.0,
                    "advisor_history_trend_status": "NEEDS_REVIEW",
                }
            ),
            encoding="utf-8",
        )
        autotune_gov_advisor_history.write_text(json.dumps({"bundle_status": "PASS"}), encoding="utf-8")
        governance_snapshot_advisor_history.write_text(json.dumps({"bundle_status": "PASS"}), encoding="utf-8")
        ...
        return (
            ...
            autotune,
            autotune_gov,
            autotune_gov_advisor_history,
            governance_snapshot_advisor_history,
            policy,
            matrix,
        )
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            proc = subprocess.run(
                [
                    ...
                    "0",
                    "--governance-snapshot-advisor-history-rc",
                    "0",
                    "--policy-dashboard-rc",
                    "0",
                    "--ci-matrix-rc",
                    ...
                    str(governance_snapshot_advisor_history),
    ...
    def test_mvp_freeze_fail(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            (
                ...
                policy,
                matrix,
            ) = self._write_artifacts(root, matrix_status="FAIL")
            out = root / "summary.json"
            proc = subprocess.run(
                [
                    "python3",
    ...
    def test_mvp_freeze_fail_on_missing_advisor_top_driver_signal(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            (
                medium,
                ...
                policy,
                matrix,
            ) = self._write_artifacts(root, matrix_status="PASS")
            autotune_gov.write_text(
                json.dumps(
                    {
                        "bundle_status": "PASS",
                        "advisor_history_latest_top_driver": "",
                        "advisor_history_top_driver_non_null_rate": 0.0,
                        "advisor_history_trend_status": "PASS",
                    }
                ),
                encoding="utf-8",
            )
            ...
            payload = json.loads(out.read_text(encoding="utf-8"))
            self.assertEqual(payload.get("verdict"), "MVP_FREEZE_FAIL")
            self.assertEqual(payload.get("blocking_step"), "advisor_top_driver_signal")
