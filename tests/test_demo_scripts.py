...
class DemoScriptTests(unittest.TestCase):
    ...
    def test_demo_invariant_repair_profile_compare_script(self) -> None:
        ...
        self.assertEqual(proc.returncode, 0, msg=proc.stderr or proc.stdout)
        payload = json.loads(
            Path("artifacts/invariant_repair_profile_compare_demo/summary.json").read_text(encoding="utf-8")
        )
        self.assertEqual(payload.get("bundle_status"), "PASS")
        self.assertGreaterEqual(
            float(payload.get("industrial_confidence_min", 0.0)),
            float(payload.get("default_confidence_min", 0.0)),
        )
    ...
    def test_demo_governance_promote_compare_script(self) -> None:
        ...
        self.assertIsInstance(payload.get("decision_ranking_top_2"), list)
        ...
        self.assertIsInstance(payload.get("decision_explanation_top_driver"), str)
        self.assertIsInstance(payload.get("decision_explanation_ranking_top_3"), list)
    ...
    def test_demo_governance_promote_apply_strict_guard_script(self) -> None:
        ...
        self.assertEqual(payload.get("with_low_quality_status"), "FAIL")
    ...
    def test_demo_governance_promote_apply_drift_script(self) -> None:
        ...
        self.assertEqual(proc.returncode, 0, msg=proc.stderr or proc.stdout)
        payload = json.loads(
            Path("artifacts/governance_promote_apply_drift_demo/summary.json").read_text(encoding="utf-8")
        )
        self.assertEqual(payload.get("bundle_status"), "PASS")
        self.assertEqual(payload.get("drift_needs_review_status"), "NEEDS_REVIEW")
    ...
    def test_demo_governance_policy_patch_apply_script(self) -> None:
        ...
        self.assertEqual(proc.returncode, 0, msg=proc.stderr or proc.stdout)
        payload = json.loads(
            Path("artifacts/governance_policy_patch_apply_demo/summary.json").read_text(encoding="utf-8")
        )
        self.assertEqual(payload.get("bundle_status"), "PASS")
        statuses = payload.get("statuses", {})
        self.assertEqual(statuses.get("no_approval"), "NEEDS_REVIEW")
        self.assertEqual(statuses.get("reject"), "FAIL")
        self.assertEqual(statuses.get("approve_hold"), "NEEDS_REVIEW")
    ...
    def test_demo_governance_policy_patch_history_script(self) -> None:
        ...
        self.assertEqual(proc.returncode, 0, msg=proc.stderr or proc.stdout)
        payload = json.loads(
            Path("artifacts/governance_policy_patch_history_demo/demo_summary.json").read_text(encoding="utf-8")
        )
        self.assertEqual(payload.get("bundle_status"), "PASS")
    ...
    def test_demo_governance_replay_bundle_script(self) -> None:
        ...
        self.assertEqual(proc.returncode, 0, msg=proc.stderr or proc.stdout)
        payload = json.loads(
            Path("artifacts/governance_replay_bundle_demo/summary.json").read_text(encoding="utf-8")
        )
        self.assertEqual(payload.get("bundle_status"), "PASS")
    ...
    def test_demo_governance_decision_bundle_script(self) -> None:
        ...
        self.assertEqual(proc.returncode, 0, msg=proc.stderr or proc.stdout)
        payload = json.loads(
            Path("artifacts/governance_decision_bundle_v1/summary.json").read_text(encoding="utf-8")
        )
        self.assertEqual(payload.get("bundle_status"), "PASS")
        self.assertIn(payload.get("compare_status"), {"PASS", "NEEDS_REVIEW", "FAIL"})
        self.assertIsInstance(payload.get("compare_best_profile"), str)
    ...
    def test_demo_mutation_dashboard_script(self) -> None:
        ...
        self.assertEqual(proc.returncode, 0, msg=proc.stderr or proc.stdout)
        payload = json.loads(Path("artifacts/mutation_dashboard_demo/summary.json").read_text(encoding="utf-8"))
        self.assertEqual(payload.get("bundle_status"), "PASS")
    ...
    def test_demo_policy_autotune_script(self) -> None:
        ...
        self.assertEqual(proc.returncode, 0, msg=proc.stderr or proc.stdout)
        payload = json.loads(Path("artifacts/policy_autotune_demo/summary.json").read_text(encoding="utf-8"))
        self.assertEqual(payload.get("bundle_status"), "PASS")
        self.assertIn(payload.get("advisor_profile"), {"default", "industrial_strict"})
        self.assertIn(payload.get("apply_final_status"), {"PASS", "NEEDS_REVIEW"})
    ...
    def test_demo_policy_autotune_governance_script(self) -> None:
        ...
        self.assertEqual(proc.returncode, 0, msg=proc.stderr or proc.stdout)
        payload = json.loads(Path("artifacts/policy_autotune_governance_demo/summary.json").read_text(encoding="utf-8"))
        self.assertEqual(payload.get("bundle_status"), "PASS")
    ...
    def test_demo_policy_autotune_governance_dashboard_script(self) -> None:
        ...
        self.assertEqual(proc.returncode, 0, msg=proc.stderr or proc.stdout)
        payload = json.loads(
            Path("artifacts/policy_autotune_governance_history_demo/demo_summary.json").read_text(encoding="utf-8")
        )
        self.assertEqual(payload.get("bundle_status"), "PASS")
        self.assertIn(payload.get("trend_status"), {"PASS", "NEEDS_REVIEW"})
