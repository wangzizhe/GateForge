...
from gateforge.agent_modelica_v0_13_0_closeout import build_v130_closeout
from gateforge.agent_modelica_v0_13_0_governance_pack import build_v130_governance_pack
from gateforge.agent_modelica_v0_13_0_handoff_integrity import build_v130_handoff_integrity
...
def _write_standard_chain(root: Path) -> dict[str, Path]:
    ...
    _write_closeout(
        paths["v123"],
        {
            ...
            "explicit_caveat_label": "bounded_operational_remedies_non_material_and_stronger_remedies_not_in_scope_after_governed_same_source_evaluation",
            "next_primary_phase_question": "capability_level_improvement_evaluation_after_operational_remedy_exhaustion",
            "do_not_continue_v0_12_same_operational_remedy_refinement_by_default": True,
        },
    )
    return paths


class AgentModelicaV130CapabilityInterventionGovernanceFlowTests(unittest.TestCase):
    def test_handoff_integrity_pass_path(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            payload = build_v130_handoff_integrity(
                v123_closeout_path=str(paths["v123"]),
    ...
    def test_baseline_anchor_pass_on_carried_v112_default_path(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(payload["baseline_anchor"]["baseline_product_gap_substrate_version"], "v0_11_2")
    ...
    def test_family_separation_ready_path_when_overlap_is_resolved(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(payload["family_separation_rule"]["family_separation_status"], "ready")
            merged = payload["family_separation_rule"]["merged_family_table"][0]
            self.assertEqual(merged["source_family"], "planner_reasoning_depth_improvement")
    ...
    def test_rejects_broad_capability_rewrite_candidate(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertIn(
                ...
                payload["capability_intervention_admission"]["rejection_reason_table"],
            )
    ...
    def test_governance_partial_when_baseline_continuity_is_broken(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(payload["capability_intervention_governance_status"], "governance_partial")
    ...
    def test_closeout_routes_to_governance_ready(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(payload["conclusion"]["version_decision"], "v0_13_0_capability_intervention_governance_ready")
            self.assertEqual(payload["conclusion"]["v0_13_1_handoff_mode"], "execute_first_bounded_capability_intervention_pack")
    ...
    def test_closeout_routes_to_invalid_on_bad_handoff(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(payload["conclusion"]["version_decision"], "v0_13_0_handoff_phase_inputs_invalid")
            self.assertEqual(payload["conclusion"]["v0_13_1_handoff_mode"], "rebuild_v0_13_0_phase_inputs_first")
