...
flags = {"status_present": "PASS" if p.get("status") in {"PASS","NEEDS_REVIEW","FAIL"} else "FAIL", "release_ready_present": "PASS" if isinstance(p.get("release_ready"), bool) else "FAIL"}
summary = {"manifest_status": p.get("status"), "release_ready": p.get("release_ready"), "result_flags": flags, "bundle_status": "PASS" if all(v=="PASS" for v in flags.values()) else "FAIL"}
