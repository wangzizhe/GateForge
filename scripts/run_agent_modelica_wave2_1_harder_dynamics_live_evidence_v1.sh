...
VARIANT_TAG="${GATEFORGE_AGENT_WAVE2_1_HARDER_DYNAMICS_VARIANT_TAG:-}"
ALLOW_PARTIAL_TASKSET="${GATEFORGE_AGENT_WAVE2_1_HARDER_DYNAMICS_ALLOW_PARTIAL_TASKSET:-0}"
...
if [ -n "$VARIANT_TAG" ]; then
  TASKSET_ARGS+=(--variant-tag "$VARIANT_TAG")
fi
if [ "$ALLOW_PARTIAL_TASKSET" = "1" ]; then
  TASKSET_ARGS+=(--allow-partial-taskset)
fi
