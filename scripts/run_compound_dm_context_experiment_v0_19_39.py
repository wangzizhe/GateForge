"""Run the compound-aware DM context experiment (v0.19.39).

Tests whether per-variable fix hints in DM context recover the performance
drop observed in v0.19.38 (B=-15pp vs A).

v0.19.38 finding: generic DM hint covering two opposite fix directions causes
LLM to make no changes at all (no-op).  This experiment replaces the generic
hint with per-variable hints:
  - PP variable: "Add a defining equation ... or restore as parameter ..."
  - PV variable: "Remove the declaration. Replace phantom with base var in equations."

Conditions:
  A: raw OMC error (same as v0.19.38 — used for matched-pair comparison)
  C: compound-aware DM context (per-variable fix hints)

Usage:
  python3 scripts/run_compound_dm_context_experiment_v0_19_39.py [--sample N]
"""
...
OUT_DIR = REPO_ROOT / "artifacts" / "compound_dm_context_experiment_v0_19_39"
...
def _run_check(model_text: str, model_name: str) -> tuple[bool, str]:
    with temporary_workspace("gf_exp39_") as ws:
        workspace = Path(ws)
...
def run_experiment(sample: int | None = None) -> None:
    ...
    for idx, case in enumerate(cases):
        ...
        # Compute DM context fresh with the updated per-variable fix hints
    ...
    summary = {
        ...
        "v0_19_38_condition_a_reference": 0.30,
        "v0_19_38_condition_b_generic_dm": 0.15,
