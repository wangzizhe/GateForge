...
LIVE_EXECUTOR_CMD="${GATEFORGE_AGENT_L4_CLOSED_LOOP_LIVE_EXECUTOR_CMD:-python3 -m gateforge.agent_modelica_live_executor_v1 --task-id \"__TASK_ID__\" --failure-type \"__FAILURE_TYPE__\" --expected-stage \"__EXPECTED_STAGE__\" --source-model-path \"__SOURCE_MODEL_PATH__\" --mutated-model-path \"__MUTATED_MODEL_PATH__\" --repair-actions __REPAIR_ACTIONS_SHQ__ --max-rounds \"__MAX_ROUNDS__\" --timeout-sec \"__MAX_TIME_SEC__\" --planner-backend \"${GATEFORGE_AGENT_L4_CLOSED_LOOP_PLANNER_BACKEND:-rule}\" --backend \"${GATEFORGE_AGENT_L4_CLOSED_LOOP_BACKEND:-openmodelica_docker}\" --docker-image \"${GATEFORGE_AGENT_L4_CLOSED_LOOP_OM_DOCKER_IMAGE:-openmodelica/openmodelica:v1.26.1-minimal}\"}"
