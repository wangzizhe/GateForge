"""Run overdetermined structural mutation experiment (v0.19.40).

Condition A: LLM sees raw OMC error (just "N+1 eq, N var" — no variable named).
Condition B: LLM sees overdetermined diagnostic context naming the conflicting
             variable and both equations.

Both conditions run fresh in same batch for matched-pair comparison.

Usage:
  python3 scripts/run_overdetermined_experiment_v0_19_40.py [--sample N]
"""
...
ADMITTED_PATH = REPO_ROOT / "artifacts" / "overdetermined_mutations_v0_19_40" / "admitted_cases.jsonl"
OUT_DIR = REPO_ROOT / "artifacts" / "overdetermined_experiment_v0_19_40"
...
def _run_check(model_text: str, model_name: str) -> tuple[bool, str]:
    with temporary_workspace("gf_exp40_") as ws:
        workspace = Path(ws)
...
def run_experiment(sample: int | None = None) -> None:
    ...
    for idx, case in enumerate(cases):
        ...
        results.append(row)
        print(f"  A={'PASS' if result_a['fix_pass'] else 'FAIL'}  "
              f"B={'PASS' if result_b['fix_pass'] else 'FAIL'}")
