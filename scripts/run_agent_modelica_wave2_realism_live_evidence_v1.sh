...
OUT_DIR="${GATEFORGE_AGENT_WAVE2_REALISM_LIVE_EVIDENCE_OUT_DIR:-artifacts/agent_modelica_wave2_realism_live_evidence_v1}"
RUN_ID="${GATEFORGE_AGENT_WAVE2_REALISM_RUN_ID:-$(python3 - <<'PY'
from datetime import datetime, timezone
print(datetime.now(timezone.utc).strftime("wave2_realism_%Y%m%dT%H%M%SZ"))
PY
)}"
RUN_ROOT="${GATEFORGE_AGENT_WAVE2_REALISM_RUN_ROOT:-$OUT_DIR/runs/$RUN_ID}"
RESUME="${GATEFORGE_AGENT_WAVE2_REALISM_RESUME:-0}"
STOP_AFTER_STAGE="${GATEFORGE_AGENT_WAVE2_REALISM_STOP_AFTER_STAGE:-${GATEFORGE_AGENT_UNKNOWN_LIBRARY_STOP_AFTER_STAGE:-}}"
...
MAX_TIME_SEC="${GATEFORGE_AGENT_WAVE2_REALISM_MAX_TIME_SEC:-180}"
RUNTIME_THRESHOLD="${GATEFORGE_AGENT_WAVE2_REALISM_RUNTIME_THRESHOLD:-0.2}"
LIVE_TIMEOUT_SEC="${GATEFORGE_AGENT_WAVE2_REALISM_LIVE_TIMEOUT_SEC:-180}"
...
UPDATE_LATEST="${GATEFORGE_AGENT_WAVE2_REALISM_UPDATE_LATEST:-1}"
...
DETERMINISTIC_EXECUTOR_CMD="${GATEFORGE_AGENT_WAVE2_DETERMINISTIC_EXECUTOR_CMD:-env GATEFORGE_AGENT_WAVE2_DETERMINISTIC_REPAIR=1 $LIVE_EXECUTOR_CMD}"
...
payload = {
    "schema_version": "agent_modelica_wave2_live_stage_status_v1",
...
Path(path).write_text(json.dumps({
    "schema_version": "agent_modelica_wave2_live_run_manifest_v1",
    "generated_at_utc": datetime.now(timezone.utc).isoformat(),
    "run_id": run_id,
    "run_root": run_root,
...
run_stage "curated_retrieval" "$CURATED_DIR/summary.json" \
  python3 -m gateforge.agent_modelica_wave2_curated_retrieval_v1 \
    --manifest "$MANIFEST_PATH" \
    --failure-types "$FAILURE_TYPES" \
...
out_path.write_text(json.dumps({"schema_version": "agent_modelica_wave2_merged_history_v1", "rows": rows}, indent=2), encoding="utf-8")
PY
...
run_stage "wave2_baseline_summary" "$RUN_ROOT/wave2_baseline_summary.json" \
  python3 -m gateforge.agent_modelica_wave2_baseline_summary_v1 \
    ...
    --out "$RUN_ROOT/wave2_baseline_summary.json"

if [ "$STOP_AFTER_STAGE" = "baseline_off_live" ] || [ "$STOP_AFTER_STAGE" = "wave2_baseline_summary" ]; then
...
run_stage "evidence" "$RUN_ROOT/decision_summary.json" \
  python3 -m gateforge.agent_modelica_wave2_evidence_v1 \
    --challenge-summary "$CHALLENGE_DIR/summary.json" \
    --baseline-summary "$BASELINE_DIR/summary.json" \
    --baseline-results "$BASELINE_DIR/results.json" \
...
payload = {
    "schema_version": "agent_modelica_wave2_live_final_run_summary_v1",
    "generated_at_utc": datetime.now(timezone.utc).isoformat(),
    "run_id": run_id,
    "run_root": run_root,
