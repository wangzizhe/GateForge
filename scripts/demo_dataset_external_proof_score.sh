...
flags = {"status_present": "PASS" if p.get("status") in {"PASS","NEEDS_REVIEW","FAIL"} else "FAIL", "score_present": "PASS" if isinstance(p.get("proof_score"), (int,float)) else "FAIL"}
summary = {"proof_status": p.get("status"), "proof_score": p.get("proof_score"), "result_flags": flags, "bundle_status": "PASS" if all(v=="PASS" for v in flags.values()) else "FAIL"}
