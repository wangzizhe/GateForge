"""A/B experiment: raw OMC output vs formatted OMC output on hard cases.

v0.19.48 discipline:
  - Identical prompt structure, model, temperature, LLM
  - Only difference: error_excerpt is raw vs formatted
  - Runs on the 5 stalled + 3 hint-resistant cases from v0.19.47

Usage:
  python3 scripts/run_formatted_signal_experiment_v0_19_48.py --mode raw
  python3 scripts/run_formatted_signal_experiment_v0_19_48.py --mode formatted
"""
...
from gateforge.omc_diagnostic_formatter_v0_19_48 import format_omc_error_excerpt
...
def _get_source_path(candidate_id: str) -> Path | None:
    """Resolve source model path from candidate_id."""
    parts = candidate_id.split("_")
    # e.g. v01945_ThermalZone_v0_pp_c1_c2_pv_phi1
    # model name is typically parts[1] + "_v0" or similar
    if len(parts) >= 3:
        model_name = parts[1]
        src_dir = REPO_ROOT / "assets_private" / "standalone_explicit_equation_source_models_v0_19_34"
        for candidate in src_dir.glob(f"{model_name}*.mo"):
            return candidate
    return None
...
def _run_check_and_simulate(model_text: str, model_name: str) -> tuple[bool, bool, str]:
    ...
    with temporary_workspace("gf_v01948_fmt_") as ws:
        workspace = Path(ws)
...
def _llm_turn(
    *,
    ...
    if mode == "formatted":
        error_excerpt = format_omc_error_excerpt(omc_output)
    else:
        error_excerpt = omc_output
...
def _run_single_case(
    ...
    for round_num in range(1, MAX_ROUNDS + 1):
        ...
        attempt = {
            ...
            "error_excerpt_mode": mode,
            "error_excerpt_length": len(omc_output if mode == "raw" else format_omc_error_excerpt(omc_output)),
        }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="A/B experiment: raw vs formatted OMC signal")
    parser.add_argument("--mode", choices=["raw", "formatted"], required=True)
    parser.add_argument("--cases", nargs="+", default=ALL_HARD_CASES, help="Specific case IDs to run")
    parser.add_argument("--out-dir", type=Path, default=REPO_ROOT / "artifacts" / "formatted_signal_experiment_v0_19_48")
    args = parser.parse_args()

    out_dir = args.out_dir
    out_dir.mkdir(parents=True, exist_ok=True)

    print(f"=== FORMATTED SIGNAL EXPERIMENT v0.19.48 ===")
