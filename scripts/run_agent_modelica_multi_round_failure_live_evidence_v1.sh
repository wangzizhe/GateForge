...
RUN_ID="${GATEFORGE_AGENT_MULTI_ROUND_FAILURE_RUN_ID:-$(python3 - <<'PY'
from datetime import datetime, timezone
print(datetime.now(timezone.utc).strftime("multi_round_failure_%Y%m%dT%H%M%SZ"))
PY
)}"
RUN_ROOT="${GATEFORGE_AGENT_MULTI_ROUND_FAILURE_RUN_ROOT:-$OUT_DIR/runs/$RUN_ID}"
RESUME="${GATEFORGE_AGENT_MULTI_ROUND_FAILURE_RESUME:-0}"
STOP_AFTER_STAGE="${GATEFORGE_AGENT_MULTI_ROUND_FAILURE_STOP_AFTER_STAGE:-}"
...
FIRST_ROUND_PASS_THRESHOLD_PCT="${GATEFORGE_AGENT_MULTI_ROUND_FIRST_ROUND_PASS_THRESHOLD_PCT:-85}"
MEDIAN_REPAIR_ROUNDS_MIN="${GATEFORGE_AGENT_MULTI_ROUND_MEDIAN_REPAIR_ROUNDS_MIN:-2}"
...
RUN_BACKEND_PREFLIGHT="${GATEFORGE_AGENT_MULTI_ROUND_RUN_BACKEND_PREFLIGHT:-1}"
UPDATE_LATEST="${GATEFORGE_AGENT_MULTI_ROUND_UPDATE_LATEST:-1}"
...
DETERMINISTIC_EXECUTOR_CMD="${GATEFORGE_AGENT_MULTI_ROUND_DETERMINISTIC_EXECUTOR_CMD:-env GATEFORGE_AGENT_MULTI_ROUND_DETERMINISTIC_REPAIR=1 $LIVE_EXECUTOR_CMD}"
RETRIEVAL_EXECUTOR_CMD="${GATEFORGE_AGENT_MULTI_ROUND_RETRIEVAL_EXECUTOR_CMD:-env GATEFORGE_AGENT_MULTI_ROUND_DETERMINISTIC_REPAIR=1 $LIVE_EXECUTOR_CMD}"
...
run_challenge_stage() {
  ...
  if [ -n "$VARIANT_TAG" ]; then
    cmd+=(--variant-tag "$VARIANT_TAG")
  fi
  if [ "$ALLOW_PARTIAL_TASKSET" = "1" ]; then
    cmd+=(--allow-partial-taskset)
  fi
  "${cmd[@]}"
}
...
run_stage "multi_round_baseline_summary" "$RUN_ROOT/multi_round_baseline_summary.json" \
  python3 -m gateforge.agent_modelica_multi_round_baseline_summary_v1 \
    ...
    --out "$RUN_ROOT/multi_round_baseline_summary.json"

python3 - "$RUN_ROOT/multi_round_baseline_summary.json" "$EASY_TASK_EXCLUSIONS_PATH" <<'PY'
import json
import sys
from datetime import datetime, timezone
from pathlib import Path
...
payload = {
    "schema_version": "agent_modelica_multi_round_easy_task_exclusions_v1",
    "generated_at_utc": datetime.now(timezone.utc).isoformat(),
    "task_ids": [item for item in (summary.get("first_round_pass_task_ids") or []) if isinstance(item, str) and item.strip()],
    "first_round_pass_pct": float(summary.get("first_round_pass_pct") or 0.0),
    "median_repair_rounds": float(summary.get("median_repair_rounds") or 0.0),
    "primary_reason": "task_construction_still_too_easy",
}
Path(sys.argv[2]).write_text(json.dumps(payload, indent=2), encoding="utf-8")
PY

if [ "$STOP_AFTER_STAGE" = "baseline_off_live" ] || [ "$STOP_AFTER_STAGE" = "multi_round_baseline_summary" ]; then
  echo "Stopped after stage: baseline_off_live"
  exit 0
fi

BASELINE_CHECK="$(python3 - <<'PY' "$RUN_ROOT/multi_round_baseline_summary.json"
import json, sys
from pathlib import Path
    ...
    json.dumps(
        {
            ...
            "median_repair_rounds": float(payload.get("median_repair_rounds") or 0.0),
        }
    )
)
PY
...
MEDIAN_REPAIR_ROUNDS="$(python3 - <<'PY' "$BASELINE_CHECK"
import json, sys
print(json.loads(sys.argv[1])["median_repair_rounds"])
PY
)"

if python3 - <<'PY' "$BASELINE_PCT" "$FIRST_ROUND_PCT" "$MEDIAN_REPAIR_ROUNDS" "$FIRST_ROUND_PASS_THRESHOLD_PCT" "$MEDIAN_REPAIR_ROUNDS_MIN"
import sys
baseline = float(sys.argv[1])
first_round = float(sys.argv[2])
median_rounds = float(sys.argv[3])
first_round_threshold = float(sys.argv[4])
median_rounds_min = float(sys.argv[5])
raise SystemExit(0 if baseline >= 100.0 and (first_round > first_round_threshold or median_rounds < median_rounds_min) else 1)
PY
then
...
run_stage "evidence" "$RUN_ROOT/evidence_summary.json" \
  python3 -m gateforge.agent_modelica_multi_round_evidence_v1 \
    --challenge-summary "$CHALLENGE_DIR/summary.json" \
    --baseline-summary "$RUN_ROOT/multi_round_baseline_summary.json" \
    --baseline-results "$BASELINE_DIR/results.json" \
...
summary = {
    "schema_version": "agent_modelica_multi_round_final_run_summary_v1",
    "generated_at_utc": datetime.now(timezone.utc).isoformat(),
    "run_id": run_id,
    "run_root": run_root,
