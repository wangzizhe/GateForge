...
(out / "demo_summary.json").write_text(json.dumps({"checkpoint_status": summary.get("status"), "milestone_decision": summary.get("milestone_decision"), "bundle_status": bundle_status, "result_flags": flags}, indent=2), encoding="utf-8")
