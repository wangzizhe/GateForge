...
OUT_DIR="${GATEFORGE_AGENT_WAVE2_REALISM_EVIDENCE_OUT_DIR:-artifacts/agent_modelica_wave2_realism_evidence_v1}"
...
REPAIR_MEMORY_PATH="${GATEFORGE_AGENT_WAVE2_REALISM_REPAIR_MEMORY:-data/private_failure_corpus/agent_modelica_repair_memory_v1.json}"
...
mkdir -p "$OUT_DIR" "$CHALLENGE_DIR" "$CURATED_DIR" "$BASELINE_DIR" "$DETERMINISTIC_DIR" "$RETRIEVAL_DIR"
...
python3 -m gateforge.agent_modelica_wave2_baseline_summary_v1 \
  ...
  --out "$OUT_DIR/wave2_baseline_summary.json"
