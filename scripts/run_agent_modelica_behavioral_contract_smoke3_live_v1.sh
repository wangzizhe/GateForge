...
OUT_DIR="${GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_SMOKE3_OUT_DIR:-artifacts/agent_modelica_behavioral_contract_smoke3_live_v1}"
RUN_ID="${GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_SMOKE3_RUN_ID:-$(python3 - <<'PY'
from datetime import datetime, timezone
print(datetime.now(timezone.utc).strftime("behavioral_contract_smoke3_%Y%m%dT%H%M%SZ"))
PY
)}"
RUN_ROOT="${GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_SMOKE3_RUN_ROOT:-$OUT_DIR/runs/$RUN_ID}"
UPDATE_LATEST="${GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_SMOKE3_UPDATE_LATEST:-1}"
...
SMOKE_TASK_IDS="${GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_SMOKE_TASK_IDS:-}"
...
PER_TASK_TIME_BUDGET_SEC="${GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_SMOKE3_PER_TASK_TIME_BUDGET_SEC:-300}"
MIN_TASKS_WITHIN_BUDGET="${GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_SMOKE3_MIN_TASKS_WITHIN_BUDGET:-2}"
...
SMOKE_CHALLENGE_SUMMARY="$RUN_ROOT/behavioral_contract_smoke_challenge_summary.json"
SMOKE_BASELINE_SUMMARY="$RUN_ROOT/behavioral_contract_baseline_summary.json"
...
Path(path).write_text(json.dumps({
    "schema_version": "agent_modelica_behavioral_contract_smoke3_run_manifest_v1",
...
counts_by_contract_family = {}
counts_by_library = {}
contract_metric_coverage = {}
for task in tasks:
    failure_type = str(task.get("failure_type") or "unknown").strip().lower()
    contract_family = str(task.get("contract_family") or "unknown").strip().lower()
    ...
    counts_by_contract_family[contract_family] = int(counts_by_contract_family.get(contract_family, 0)) + 1
    counts_by_library[library_id] = int(counts_by_library.get(library_id, 0)) + 1
    for metric in [str(item).strip() for item in (task.get("contract_metric_set") or []) if str(item).strip()]:
        contract_metric_coverage[metric] = int(contract_metric_coverage.get(metric, 0)) + 1
summary = {
    "schema_version": "agent_modelica_behavioral_contract_smoke_challenge_summary_v1",
    "generated_at_utc": datetime.now(timezone.utc).isoformat(),
    ...
    "contract_metric_coverage": contract_metric_coverage,
}
...
summary = {
    "schema_version": "agent_modelica_behavioral_contract_smoke3_final_run_summary_v1",
    "generated_at_utc": datetime.now(timezone.utc).isoformat(),
    "run_id": run_id,
    "run_root": run_root,
    ...
    "paths": {
        ...
        "behavioral_contract_baseline_summary": str(Path(run_root) / "behavioral_contract_baseline_summary.json"),
    },
}
