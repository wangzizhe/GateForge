...
OUT_DIR="artifacts/dataset_promotion_candidate_apply_history_demo"
mkdir -p "$OUT_DIR"
...
run_dep_script "scripts/demo_dataset_promotion_candidate_apply.sh" "artifacts/dataset_promotion_candidate_apply_demo/apply_pass.json"

python3 -m gateforge.dataset_promotion_candidate_apply_history \
  --record artifacts/dataset_promotion_candidate_apply_demo/apply_needs_review.json \
  --record artifacts/dataset_promotion_candidate_apply_demo/apply_pass.json \
...
python3 -m gateforge.dataset_promotion_candidate_apply_history_trend \
...
out = Path("artifacts/dataset_promotion_candidate_apply_history_demo")
...
flags = {
    ...
    "history_status_counts_present": "PASS" if isinstance(history.get("status_counts"), dict) else "FAIL",
...
summary = {
    ...
    "history_latest_final_status": history.get("latest_final_status"),
    "history_fail_rate": history.get("fail_rate"),
...
(out / "summary.md").write_text(
    "\n".join(
        [
            "# Dataset Promotion Candidate Apply History Demo",
            "",
            f"- history_total_records: `{summary['history_total_records']}`",
            f"- history_latest_final_status: `{summary['history_latest_final_status']}`",
            f"- history_fail_rate: `{summary['history_fail_rate']}`",
