...
OUT_DIR="artifacts/dataset_strategy_autotune_apply_history_demo"
mkdir -p "$OUT_DIR"
...
run_dep_script "scripts/demo_dataset_strategy_autotune_apply.sh" "artifacts/dataset_strategy_autotune_apply_demo/apply_pass.json"

python3 -m gateforge.dataset_strategy_autotune_apply_history \
  --record artifacts/dataset_strategy_autotune_apply_demo/apply_needs_review.json \
  --record artifacts/dataset_strategy_autotune_apply_demo/apply_pass.json \
...
python3 -m gateforge.dataset_strategy_autotune_apply_history_trend \
...
out = Path("artifacts/dataset_strategy_autotune_apply_history_demo")
...
(out / "summary.md").write_text(
    "\n".join(
        [
            "# Dataset Strategy Auto-Tune Apply History Demo",
            "",
