...
OUT_DIR="${GATEFORGE_AGENT_WAVE2_1_SMOKE3_OUT_DIR:-artifacts/agent_modelica_wave2_1_smoke3_live_v1}"
RUN_ID="${GATEFORGE_AGENT_WAVE2_1_SMOKE3_RUN_ID:-$(python3 - <<'PY'
from datetime import datetime, timezone
print(datetime.now(timezone.utc).strftime("wave2_1_smoke3_%Y%m%dT%H%M%SZ"))
PY
)}"
RUN_ROOT="${GATEFORGE_AGENT_WAVE2_1_SMOKE3_RUN_ROOT:-$OUT_DIR/runs/$RUN_ID}"
UPDATE_LATEST="${GATEFORGE_AGENT_WAVE2_1_SMOKE3_UPDATE_LATEST:-1}"
...
SMOKE_TASK_IDS="${GATEFORGE_AGENT_WAVE2_1_SMOKE_TASK_IDS:-wave2_1_buildings_loads_solver_sensitive_simulate_failure,wave2_1_ibpsa_acsimplegrid_event_logic_error,wave2_1_transform_integratorwithreset_semantic_drift_after_compile_pass}"
...
MAX_TIME_SEC="${GATEFORGE_AGENT_WAVE2_1_HARDER_DYNAMICS_MAX_TIME_SEC:-300}"
RUNTIME_THRESHOLD="${GATEFORGE_AGENT_WAVE2_1_HARDER_DYNAMICS_RUNTIME_THRESHOLD:-0.2}"
LIVE_TIMEOUT_SEC="${GATEFORGE_AGENT_WAVE2_1_HARDER_DYNAMICS_LIVE_TIMEOUT_SEC:-300}"
LIVE_MAX_OUTPUT_CHARS="${GATEFORGE_AGENT_WAVE2_1_HARDER_DYNAMICS_LIVE_MAX_OUTPUT_CHARS:-2400}"
PER_TASK_TIME_BUDGET_SEC="${GATEFORGE_AGENT_WAVE2_1_SMOKE3_PER_TASK_TIME_BUDGET_SEC:-300}"
MIN_TASKS_WITHIN_BUDGET="${GATEFORGE_AGENT_WAVE2_1_SMOKE3_MIN_TASKS_WITHIN_BUDGET:-2}"
...
payload = {
    "schema_version": "agent_modelica_wave2_1_smoke3_stage_status_v1",
...
payload = {
    "schema_version": "agent_modelica_wave2_1_smoke3_run_manifest_v1",
    "generated_at_utc": datetime.now(timezone.utc).isoformat(),
    "run_id": run_id,
    "out_dir": out_dir,
    "run_root": run_root,
...
run_backend_preflight_stage() {
  if [ "$RUN_BACKEND_PREFLIGHT" != "1" ]; then
    write_json_file "$BACKEND_PREFLIGHT_PATH" '{"schema_version":"agent_modelica_wave2_1_backend_preflight_v1","status":"SKIPPED","reason":"backend_preflight_disabled"}'
    return 0
  fi
...
summary = {
    "schema_version": "agent_modelica_wave2_1_smoke3_final_run_summary_v1",
    "generated_at_utc": datetime.now(timezone.utc).isoformat(),
    "run_id": run_id,
    "run_root": run_root,
