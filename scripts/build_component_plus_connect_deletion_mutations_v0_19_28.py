"""Build compound structural deletions for v0.19.28.

Goal:
  - Push beyond the single-fix boundary observed for pure passive-component
    declaration deletion in v0.19.27.
  - Create a more realistic refactor-error compound: delete one passive
    component declaration and one adjacent non-ground, non-sensor connect line.

Design:
  - Stay on the same existing MSL source models.
  - Only use passive components (R/C/L).
  - The extra deleted connect must directly reference the deleted component and
    must not be a ground or sensor connect.
  - Admission requires source PASS and mutant FAIL at checkModel.
"""
...
OUT_DIR = REPO_ROOT / "artifacts" / "component_plus_connect_deletion_mutations_v0_19_28"
...
@dataclass(frozen=True)
class CompoundDeletionCandidate:
    source_file: str
    model_name: str
    candidate_id: str
    component: ComponentRow
    connect: ConnectRow
    incident_connect_count: int
...
def _extract_incident_non_sensor_connects(model_text: str, instance: str) -> list[ConnectRow]:
    prefix = f"{instance}."
    ...
    for idx, line in enumerate(model_text.splitlines()):
        ...
        if not (lhs.startswith(prefix) or rhs.startswith(prefix)):
            continue
...
def _build_candidates() -> list[CompoundDeletionCandidate]:
    rows: list[CompoundDeletionCandidate] = []
    for source_path, model_name, source_text in _load_source_models():
        order = 0
        for component in _extract_passive_components(source_text):
            incident = _extract_incident_non_sensor_connects(source_text, component.instance)
            if not incident:
                continue
            for connect in incident:
                order += 1
                rows.append(
                    CompoundDeletionCandidate(
                        source_file=source_path.name,
                        model_name=model_name,
                        candidate_id=(
                            f"v01928_{source_path.stem}_compconn_{order:02d}_"
                            f"{_sanitize_token(component.instance)}"
                        ),
                        component=component,
                        connect=connect,
                        incident_connect_count=len(incident),
                    )
                )
    return rows


def _build_mutated_text(source_text: str, component_idx: int, connect_idx: int) -> str:
    lines = source_text.splitlines()
    for idx in sorted([component_idx, connect_idx], reverse=True):
        del lines[idx]
    return "\n".join(lines) + "\n"
...
def _admit_candidate(spec: CompoundDeletionCandidate, source_text: str, precheck: dict) -> dict | None:
    ...
    mutated_text = _build_mutated_text(source_text, spec.component.index, spec.connect.index)
    ...
    return {
        ...
        "workflow_goal": (
            "Repair the structural refactor mistake so the Modelica model passes "
            "checkModel again while preserving the original circuit intent."
        ),
        "failure_type": failure_type,
        "expected_stage": "check",
        "mutation_family": "component_plus_connect_deletion_reference",
        "mutation_mechanism": "deleted_passive_component_and_adjacent_connect",
        ...
        "deleted_connect_line": spec.connect.line,
        "deleted_connect_kind": spec.connect.kind,
        "deleted_component_incident_non_sensor_connect_count": spec.incident_connect_count,
        "planner_backend": "auto",
        "backend": "openmodelica_docker",
        "admission_source": "omc_component_plus_connect_deletion_check_verified",
...
def main() -> None:
    ...
    for spec in _build_candidates():
        ...
        kkind = str(row["deleted_connect_kind"])
        by_connect_kind[kkind] = by_connect_kind.get(kkind, 0) + 1
    ...
    summary = {
        ...
        "admitted_by_deleted_connect_kind": by_connect_kind,
