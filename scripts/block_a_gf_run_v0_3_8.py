...
from gateforge.agent_modelica_branch_switch_forcing_family_spec_v0_3_8 import (  # noqa: E402
    BASELINE_LEVER_NAME,
    BASELINE_PROTOCOL_VERSION,
    BASELINE_REFERENCE_VERSION,
)
from gateforge.agent_modelica_branch_switch_forcing_taskset_v0_3_8 import (  # noqa: E402
...
DEFAULT_TASKSET_DIR = REPO_ROOT / "artifacts" / "agent_modelica_branch_switch_forcing_taskset_v0_3_8"
DEFAULT_RESULTS_DIR = REPO_ROOT / "artifacts" / "agent_modelica_block_a_gf_results_v0_3_8"
...
def _load_taskset(taskset_path: pathlib.Path) -> list[dict]:
    if not taskset_path.exists():
        build_branch_switch_forcing_taskset(out_dir=str(DEFAULT_TASKSET_DIR))
...
def run_one(task: dict, out_dir: pathlib.Path) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="gf_v038_run_") as td:
        tmp = pathlib.Path(td)
        ...
        runtime_context = AgentModelicaRuntimeContext.create(
            task_id=task_id,
            run_id=f"{task_id}_v038_branch_switch_forcing_baseline",
            arm_kind="gateforge",
            profile_id="repair-executor",
        ...
        if result_json:
            ...
            return {
                ...
                "current_branch": task.get("current_branch"),
                "preferred_branch": task.get("preferred_branch"),
                "branch_forcing_design": task.get("branch_forcing_design"),
                "baseline_measurement_protocol": runtime_context.baseline_measurement_protocol,
            }
...
def main() -> int:
    _load_dotenv()
    parser = argparse.ArgumentParser(description="Run the v0.3.8 Block A baseline authority setup.")
    ...
    if not tasks:
        print("ERROR: No tasks available for v0.3.8 Block A run.")
        return 1
