...
{"status": "PASS", "recommendation": "EXECUTE_BASE_PLAN"}
JSON
...
flags = {"status_present": "PASS" if p.get("status") in {"PASS","NEEDS_REVIEW","FAIL"} else "FAIL", "decision_present": "PASS" if isinstance(p.get("decision"), str) and p.get("decision") else "FAIL"}
summary = {"proofbook_status": p.get("status"), "decision": p.get("decision"), "result_flags": flags, "bundle_status": "PASS" if all(v=="PASS" for v in flags.values()) else "FAIL"}
