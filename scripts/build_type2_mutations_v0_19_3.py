"""
Build and OMC-verify Type 2 inter-layer multi-turn mutations for v0.19.3.
...
Outputs:
  artifacts/type2_mutations_v0_19_3/
    candidates.jsonl
    summary.json
...
OUT_DIR = REPO_ROOT / "artifacts" / "type2_mutations_v0_19_3"
DOCKER_IMAGE = "openmodelica/openmodelica:v1.26.1-minimal"

# Layer 1 placeholder: an undefined variable name that will cause checkModel to fail
L1_UNDEF_VAR = "gateforge_undef_trigger_v2"

# Injection lines (inserted at the start of the equation section)
_L1_ASSERT = (
    f'  assert({L1_UNDEF_VAR} > 0.0, "type2_l1_undef_check");'
)
...
def _remove_l1(text: str) -> str:
    """Remove the Layer 1 assert line (after agent fixes undefined-var error)."""
    lines = [l for l in text.splitlines() if L1_UNDEF_VAR not in l]
    return "\n".join(lines) + "\n"
...
def _verify_candidate(spec: dict, source_text: str, source_path: Path) -> dict:
    ...
    if L1_UNDEF_VAR not in mut_check_err:
        return {
            "ok": False,
            "reason": f"expected '{L1_UNDEF_VAR}' in checkModel error, got: {mut_check_err[:200]}",
        }
    print(f"      ✓ mutated checkModel fails: mentions '{L1_UNDEF_VAR}'")

    print(f"  [C] Removing L1 (undefined assert)...")
    l1_fixed_text = _remove_l1(mutated_text)
