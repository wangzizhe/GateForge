...
OUT_DIR="${GATEFORGE_AGENT_CURATED_HARD_UNSEEN_EVIDENCE_OUT_DIR:-artifacts/agent_modelica_curated_hard_unseen_evidence_v1}"
...
REPAIR_MEMORY_PATH="${GATEFORGE_AGENT_CURATED_HARD_UNSEEN_REPAIR_MEMORY:-data/private_failure_corpus/agent_modelica_repair_memory_v1.json}"
FAILURE_TYPES="${GATEFORGE_AGENT_CURATED_HARD_UNSEEN_FAILURE_TYPES:-underconstrained_system,connector_mismatch,initialization_infeasible}"
HOLDOUT_RATIO="${GATEFORGE_AGENT_CURATED_HARD_UNSEEN_HOLDOUT_RATIO:-0.15}"
SPLIT_SEED="${GATEFORGE_AGENT_CURATED_HARD_UNSEEN_SPLIT_SEED:-agent_modelica_curated_hard_unseen_taskset_v1}"
MIN_RETRIEVAL_COVERAGE_PCT="${GATEFORGE_AGENT_CURATED_HARD_UNSEEN_MIN_RETRIEVAL_COVERAGE_PCT:-50.0}"
MIN_DIAGNOSTIC_PARSE_COVERAGE_PCT="${GATEFORGE_AGENT_CURATED_HARD_UNSEEN_MIN_DIAGNOSTIC_PARSE_COVERAGE_PCT:-95.0}"
...
ARGS=(
  python3 -m gateforge.agent_modelica_curated_hard_unseen_taskset_v1
...
python3 -m gateforge.agent_modelica_curated_hard_unseen_curated_retrieval_v1 \
  --manifest "$MANIFEST_PATH" \
  --failure-types "$FAILURE_TYPES" \
...
out_path.write_text(json.dumps({"schema_version": "agent_modelica_curated_hard_unseen_merged_history_v1", "rows": rows}, indent=2), encoding="utf-8")
PY
...
python3 -m gateforge.agent_modelica_curated_hard_unseen_baseline_summary_v1 \
  ...
  --out "$OUT_DIR/hard_unseen_baseline_summary.json" \
  --source-unstable-exclusions-out "$OUT_DIR/source_unstable_exclusions.json"
