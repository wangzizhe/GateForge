...
OUT_DIR="${GATEFORGE_AGENT_UNKNOWN_LIBRARY_SMOKE3_OUT_DIR:-artifacts/agent_modelica_unknown_library_smoke3_live_v1}"
RUN_ID="${GATEFORGE_AGENT_UNKNOWN_LIBRARY_SMOKE3_RUN_ID:-$(python3 - <<'PY'
from datetime import datetime, timezone
print(datetime.now(timezone.utc).strftime("unknownlib_smoke3_%Y%m%dT%H%M%SZ"))
PY
)}"
RUN_ROOT="${GATEFORGE_AGENT_UNKNOWN_LIBRARY_SMOKE3_RUN_ROOT:-$OUT_DIR/runs/$RUN_ID}"
UPDATE_LATEST="${GATEFORGE_AGENT_UNKNOWN_LIBRARY_SMOKE3_UPDATE_LATEST:-1}"
...
SMOKE_TASK_IDS="${GATEFORGE_AGENT_UNKNOWN_LIBRARY_SMOKE_TASK_IDS:-unknownlib_aixlib_simplehouse_connector_mismatch,unknownlib_openipsl_loadtestpq_connector_mismatch,unknownlib_transform_simplebattery_test_connector_mismatch}"
...
MAX_TIME_SEC="${GATEFORGE_AGENT_UNKNOWN_LIBRARY_MAX_TIME_SEC:-300}"
RUNTIME_THRESHOLD="${GATEFORGE_AGENT_UNKNOWN_LIBRARY_RUNTIME_THRESHOLD:-0.2}"
LIVE_TIMEOUT_SEC="${GATEFORGE_AGENT_UNKNOWN_LIBRARY_LIVE_TIMEOUT_SEC:-300}"
LIVE_MAX_OUTPUT_CHARS="${GATEFORGE_AGENT_UNKNOWN_LIBRARY_LIVE_MAX_OUTPUT_CHARS:-2400}"
PER_TASK_TIME_BUDGET_SEC="${GATEFORGE_AGENT_UNKNOWN_LIBRARY_SMOKE3_PER_TASK_TIME_BUDGET_SEC:-300}"
MIN_TASKS_WITHIN_BUDGET="${GATEFORGE_AGENT_UNKNOWN_LIBRARY_SMOKE3_MIN_TASKS_WITHIN_BUDGET:-2}"
...
stage_update() {
  ...
  python3 - "$RUN_ROOT" "$stage" "$status" "$summary_path" "$details_json" <<'PY'
import json
import sys
from datetime import datetime, timezone
from pathlib import Path
run_root, stage, status, summary_path, details_json = sys.argv[1:]
payload = {
    "schema_version": "agent_modelica_unknown_library_smoke3_stage_status_v1",
...
run_stage() {
  ...
  local func_name="$3"
  local details_json="${4:-{}}"
  stage_update "$stage" "RUNNING" "$summary_path" "$details_json"
  set +e
  "$func_name"
  local rc=$?
  set -e
  if [ "$rc" -ne 0 ]; then
    stage_update "$stage" "FAIL" "$summary_path" "$details_json"
    exit "$rc"
  fi
  local status
  status="$(summary_status "$summary_path")"
  stage_update "$stage" "$status" "$summary_path" "$details_json"
}
...
payload = {
    "schema_version": "agent_modelica_unknown_library_smoke3_run_manifest_v1",
    "generated_at_utc": datetime.now(timezone.utc).isoformat(),
    "run_id": run_id,
    "out_dir": out_dir,
    "run_root": run_root,
...
run_backend_preflight_stage() {
  if [ "$RUN_BACKEND_PREFLIGHT" != "1" ]; then
    write_json_file "$BACKEND_PREFLIGHT_PATH" '{"schema_version":"agent_modelica_unknown_library_backend_preflight_v1","status":"SKIPPED","reason":"backend_preflight_disabled"}'
    return 0
  fi
...
smoke = {}
path = Path(smoke_summary_path)
if path.exists():
    smoke = json.loads(path.read_text(encoding="utf-8"))
summary = {
    "schema_version": "agent_modelica_unknown_library_smoke3_final_run_summary_v1",
    "generated_at_utc": datetime.now(timezone.utc).isoformat(),
    "run_id": run_id,
    "run_root": run_root,
...
for name, payload in (
    ("latest_run.json", {"run_id": run_id, "run_root": run_root, "status": summary["status"], "summary_path": str(final)}),
    ("latest_summary.json", summary),
):
    (out_root / name).write_text(json.dumps(payload, indent=2), encoding="utf-8")
PY
}
