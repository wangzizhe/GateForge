...
payload = {
    ...
    "generated_at_utc": datetime.now(timezone.utc).isoformat(),
    "task_ids": [item for item in (summary.get("trivial_restore_suspected_task_ids") or []) if isinstance(item, str) and item.strip()],
...
if python3 - <<'PY' "$BASELINE_PCT" "$TRIVIAL_PCT" "$FIRST_ROUND_PCT" "$TRIVIAL_RESTORE_THRESHOLD_PCT" "$FIRST_ROUND_PASS_THRESHOLD_PCT"
import sys
baseline = float(sys.argv[1])
trivial = float(sys.argv[2])
first_round = float(sys.argv[3])
trivial_threshold = float(sys.argv[4])
first_round_threshold = float(sys.argv[5])
raise SystemExit(0 if baseline >= 100.0 and (trivial > trivial_threshold or first_round > first_round_threshold) else 1)
PY
then
