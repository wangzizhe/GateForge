...
OUT_DIR="${GATEFORGE_AGENT_WAVE2_2_SMOKE3_OUT_DIR:-artifacts/agent_modelica_wave2_2_smoke3_live_v1}"
RUN_ID="${GATEFORGE_AGENT_WAVE2_2_SMOKE3_RUN_ID:-$(python3 - <<'PY'
from datetime import datetime, timezone
print(datetime.now(timezone.utc).strftime("wave2_2_smoke3_%Y%m%dT%H%M%SZ"))
PY
)}"
RUN_ROOT="${GATEFORGE_AGENT_WAVE2_2_SMOKE3_RUN_ROOT:-$OUT_DIR/runs/$RUN_ID}"
UPDATE_LATEST="${GATEFORGE_AGENT_WAVE2_2_SMOKE3_UPDATE_LATEST:-1}"
...
SMOKE_TASK_IDS="${GATEFORGE_AGENT_WAVE2_2_SMOKE_TASK_IDS:-}"
...
MAX_TIME_SEC="${GATEFORGE_AGENT_WAVE2_2_COUPLED_HARD_MAX_TIME_SEC:-300}"
RUNTIME_THRESHOLD="${GATEFORGE_AGENT_WAVE2_2_COUPLED_HARD_RUNTIME_THRESHOLD:-0.2}"
LIVE_TIMEOUT_SEC="${GATEFORGE_AGENT_WAVE2_2_COUPLED_HARD_LIVE_TIMEOUT_SEC:-300}"
LIVE_MAX_OUTPUT_CHARS="${GATEFORGE_AGENT_WAVE2_2_COUPLED_HARD_LIVE_MAX_OUTPUT_CHARS:-2400}"
PER_TASK_TIME_BUDGET_SEC="${GATEFORGE_AGENT_WAVE2_2_SMOKE3_PER_TASK_TIME_BUDGET_SEC:-300}"
MIN_TASKS_WITHIN_BUDGET="${GATEFORGE_AGENT_WAVE2_2_SMOKE3_MIN_TASKS_WITHIN_BUDGET:-2}"
...
payload = {
    "schema_version": "agent_modelica_wave2_2_smoke3_stage_status_v1",
...
payload = {
    "schema_version": "agent_modelica_wave2_2_smoke3_run_manifest_v1",
    "generated_at_utc": datetime.now(timezone.utc).isoformat(),
    "run_id": run_id,
    "out_dir": out_dir,
    "run_root": run_root,
...
run_backend_preflight_stage() {
  if [ "$RUN_BACKEND_PREFLIGHT" != "1" ]; then
    write_json_file "$BACKEND_PREFLIGHT_PATH" '{"schema_version":"agent_modelica_wave2_2_backend_preflight_v1","status":"SKIPPED","reason":"backend_preflight_disabled"}'
    return 0
  fi
...
def library_key(row):
    ...
    return str(
        source_meta.get("library_id")
        or row.get("source_library")
...
def model_key(row):
    source_meta = row.get("source_meta") if isinstance(row.get("source_meta"), dict) else {}
    return str(
        source_meta.get("model_id")
        or row.get("source_library_model_path")
        or row.get("source_qualified_model_name")
        or row.get("model_id")
        or ""
    ).strip().lower()
...
for failure_type in [
    ...
    candidates.sort(
        key=lambda row: (
            ...
            order.get(str(row.get("repair_triviality_risk") or "").strip().lower(), 9),
            -int(row.get("source_dependency_count") or 0),
...
summary = {
    "schema_version": "agent_modelica_wave2_2_smoke3_final_run_summary_v1",
    "generated_at_utc": datetime.now(timezone.utc).isoformat(),
    "run_id": run_id,
    "run_root": run_root,
