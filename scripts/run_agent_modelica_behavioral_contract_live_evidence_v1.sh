...
OUT_DIR="${GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_LIVE_EVIDENCE_OUT_DIR:-artifacts/agent_modelica_behavioral_contract_live_evidence_v1}"
RUN_ID="${GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_RUN_ID:-$(python3 - <<'PY'
from datetime import datetime, timezone
print(datetime.now(timezone.utc).strftime("behavioral_contract_%Y%m%dT%H%M%SZ"))
PY
)}"
RUN_ROOT="${GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_RUN_ROOT:-$OUT_DIR/runs/$RUN_ID}"
RESUME="${GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_RESUME:-0}"
STOP_AFTER_STAGE="${GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_STOP_AFTER_STAGE:-}"
UPDATE_LATEST="${GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_UPDATE_LATEST:-1}"
...
MAX_ROUNDS="${GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_MAX_ROUNDS:-3}"
MAX_TIME_SEC="${GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_MAX_TIME_SEC:-300}"
RUNTIME_THRESHOLD="${GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_RUNTIME_THRESHOLD:-0.2}"
LIVE_TIMEOUT_SEC="${GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_LIVE_TIMEOUT_SEC:-300}"
LIVE_MAX_OUTPUT_CHARS="${GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_LIVE_MAX_OUTPUT_CHARS:-2400}"
CONTRACT_PASS_TOO_EASY_THRESHOLD_PCT="${GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_PASS_TOO_EASY_THRESHOLD_PCT:-95}"
RUN_BACKEND_PREFLIGHT="${GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_RUN_BACKEND_PREFLIGHT:-1}"
...
DETERMINISTIC_EXECUTOR_CMD="${GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_DETERMINISTIC_EXECUTOR_CMD:-env GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_DETERMINISTIC_REPAIR=1 $LIVE_EXECUTOR_CMD}"
RETRIEVAL_EXECUTOR_CMD="${GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_RETRIEVAL_EXECUTOR_CMD:-env GATEFORGE_AGENT_BEHAVIORAL_CONTRACT_DETERMINISTIC_REPAIR=1 $LIVE_EXECUTOR_CMD}"
...
payload = {
    "schema_version": "agent_modelica_behavioral_contract_live_stage_status_v1",
...
Path(path).write_text(json.dumps({
    "schema_version": "agent_modelica_behavioral_contract_live_run_manifest_v1",
    "generated_at_utc": datetime.now(timezone.utc).isoformat(),
    "run_id": run_id,
    "run_root": run_root,
...
run_stage "behavioral_contract_baseline_summary" "$RUN_ROOT/behavioral_contract_baseline_summary.json" \
  python3 -m gateforge.agent_modelica_behavioral_contract_baseline_summary_v1 \
    ...
    --out "$RUN_ROOT/behavioral_contract_baseline_summary.json"

if [ "$STOP_AFTER_STAGE" = "baseline_off_live" ] || [ "$STOP_AFTER_STAGE" = "behavioral_contract_baseline_summary" ]; then
  echo "Stopped after stage: baseline_off_live"
  exit 0
fi

BASELINE_CHECK="$(python3 - <<'PY' "$RUN_ROOT/behavioral_contract_baseline_summary.json"
import json
import sys
from pathlib import Path
payload = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
print(json.dumps({
    "contract_pass_pct": float(payload.get("contract_pass_pct") or 0.0),
    "contract_fail_count": int(payload.get("contract_fail_count") or 0),
}))
PY
)"

CONTRACT_PASS_PCT="$(python3 - <<'PY' "$BASELINE_CHECK"
import json, sys
print(json.loads(sys.argv[1])["contract_pass_pct"])
PY
)"
CONTRACT_FAIL_COUNT="$(python3 - <<'PY' "$BASELINE_CHECK"
import json, sys
print(json.loads(sys.argv[1])["contract_fail_count"])
PY
)"

if python3 - <<'PY' "$CONTRACT_PASS_PCT" "$CONTRACT_FAIL_COUNT" "$CONTRACT_PASS_TOO_EASY_THRESHOLD_PCT"
import sys
contract_pass_pct = float(sys.argv[1])
contract_fail_count = int(sys.argv[2])
threshold = float(sys.argv[3])
raise SystemExit(0 if contract_pass_pct >= threshold and contract_fail_count <= 1 else 1)
PY
then
  printf '{\n  "status": "SKIPPED",\n  "success_count": 0,\n  "success_at_k_pct": 0.0,\n  "contract_pass_pct": 0.0\n}\n' > "$DETERMINISTIC_DIR/summary.json"
...
out_path.write_text(json.dumps({"schema_version": "agent_modelica_behavioral_contract_retrieval_history_v1", "rows": rows}, indent=2), encoding="utf-8")
PY
...
run_stage "evidence" "$RUN_ROOT/evidence_summary.json" \
  python3 -m gateforge.agent_modelica_behavioral_contract_evidence_v1 \
    --challenge-summary "$CHALLENGE_DIR/summary.json" \
    --baseline-summary "$RUN_ROOT/behavioral_contract_baseline_summary.json" \
    --baseline-results "$BASELINE_DIR/results.json" \
...
summary = {
    "schema_version": "agent_modelica_behavioral_contract_final_run_summary_v1",
    "generated_at_utc": datetime.now(timezone.utc).isoformat(),
    "run_id": run_id,
    "run_root": run_root,
...
if str(update_latest) == "1":
    out_dir = Path(run_root).parents[1]
    (out_dir / "latest_run.json").write_text(json.dumps({"run_id": run_id, "run_root": run_root}, indent=2), encoding="utf-8")
    for name in ("evidence_summary.json", "gate_summary.json", "decision_summary.json", "final_run_summary.json"):
        src = Path(run_root) / name
        if src.exists():
            (out_dir / f"latest_{name}").write_text(src.read_text(encoding="utf-8"), encoding="utf-8")
PY
