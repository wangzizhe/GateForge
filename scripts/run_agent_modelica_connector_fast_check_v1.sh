...
TIMEOUT_SEC="${GATEFORGE_AGENT_CONNECTOR_FAST_CHECK_TIMEOUT_SEC:-120}"

mkdir -p "$OUT_DIR"
export GATEFORGE_OM_DOCKER_LIBRARY_CACHE="${GATEFORGE_OM_DOCKER_LIBRARY_CACHE:-$ROOT_DIR/$OUT_DIR/.gf_omcache/libraries}"
mkdir -p "$GATEFORGE_OM_DOCKER_LIBRARY_CACHE"
