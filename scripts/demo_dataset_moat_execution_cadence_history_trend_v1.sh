...
OUT_DIR="artifacts/dataset_moat_execution_cadence_history_trend_v1_demo"
mkdir -p "$OUT_DIR"
rm -f "$OUT_DIR"/*.json "$OUT_DIR"/*.md

cat > "$OUT_DIR/previous.json" <<'JSON'
{"status":"PASS","avg_execution_cadence_score":79.0,"avg_weekly_model_target":3.2}
JSON

cat > "$OUT_DIR/current.json" <<'JSON'
{"status":"NEEDS_REVIEW","avg_execution_cadence_score":73.0,"avg_weekly_model_target":2.6}
JSON

python3 -m gateforge.dataset_moat_execution_cadence_history_trend_v1 \
...
out = Path("artifacts/dataset_moat_execution_cadence_history_trend_v1_demo")
