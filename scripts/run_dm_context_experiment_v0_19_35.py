"""Run the DM diagnostic context experiment (v0.19.35).

Compares two conditions on parameter_promotion cases only:
...
Usage:
  python3 scripts/run_dm_context_experiment_v0_19_35.py [--sample N]
"""
...
# 38 cases × 2 conditions = 76 calls
os.environ.setdefault("GATEFORGE_AGENT_LIVE_MAX_REQUESTS_PER_RUN", "200")
...
OUT_DIR = REPO_ROOT / "artifacts" / "dm_context_experiment_v0_19_35"
DOCKER_IMAGE = "openmodelica/openmodelica:v1.26.1-minimal"
PLANNER_BACKEND = "gemini"
MUTATION_TYPE_FILTER = "parameter_promotion"
...
def _run_check(model_text: str, model_name: str) -> tuple[bool, str]:
    with temporary_workspace("gf_exp35_") as ws:
        workspace = Path(ws)
...
def run_experiment(sample: int | None = None) -> None:
    ...
    for idx, case in enumerate(cases):
        ...
        # Compute DM context fresh from mutated model
        dm_ctx = build_dm_diagnostic_context(broken_text)

        print(f"[{idx + 1}/{len(cases)}] {cid}")
        print(f"  DM root cause: {dm_ctx.splitlines()[2] if len(dm_ctx.splitlines()) > 2 else '?'}")
        ...
        # Save patched model texts for failure analysis
        ...
        row = {
            ...
            "dm_root_cause": _extract_root_cause(dm_ctx),
...
def _extract_root_cause(dm_ctx: str) -> str:
    """Pull root cause variable name from DM context string."""
    for line in dm_ctx.splitlines():
        if "Root cause variable" in line:
            # "Root cause variable (no defining equation): Tr  "reset time constant""
            parts = line.split(":")
            if len(parts) >= 2:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sample", type=int, default=None,
                        help="Limit to N cases (default: all parameter_promotion)")
